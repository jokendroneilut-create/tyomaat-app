...
export async function POST(request: Request) {
  try {
    ...
    const isVeteliKaava = normalize(metadata.resolver) === "vetelikaavaresolver"
    const isMultiaKaava = normalize(metadata.resolver) === "multiakaavaresolver"
    const isPetajavesiKaava = normalize(metadata.resolver) === "petajavesikaavaresolver"
    const isPihtipudasKaava = normalize(metadata.resolver) === "pihtipudaskaavaresolver"
    const isToivakkaKaava = normalize(metadata.resolver) === "toivakkakaavaresolver"
    const isUurainenKaava = normalize(metadata.resolver) === "uurainenkaavaresolver"
    const isViitasaariKaava = normalize(metadata.resolver) === "viitasaarikaavaresolver"
    const isIittiKaava = normalize(metadata.resolver) === "iittikaavaresolver"
    const isMiehikkalaKaava = normalize(metadata.resolver) === "miehikkalakaavaresolver"
    const isPyhtaaKaava = normalize(metadata.resolver) === "pyhtaakaavaresolver"
    const isPornainenKaava = normalize(metadata.resolver) === "pornainenkaavaresolver"
    const isHankoKaava = normalize(metadata.resolver) === "hankokaavaresolver"
    const isInkooKaava = normalize(metadata.resolver) === "inkookaavaresolver"
    const isKarkkilaKaava = normalize(metadata.resolver) === "karkkilakaavaresolver"
    const isSiuntioKaava = normalize(metadata.resolver) === "siuntiokaavaresolver"
    const isEuraKaava = normalize(metadata.resolver) === "eurakaavaresolver"
    const isSiikainenKaava = normalize(metadata.resolver) === "siikainenkaavaresolver"
    const isJoutsaKaava = normalize(metadata.resolver) === "joutsakaavaresolver"
    const isPielavesiKaava = normalize(metadata.resolver) === "pielavesikaavaresolver"
    const isKiuruvesiKaava = normalize(metadata.resolver) === "kiuruvesikaavaresolver"
    const isAuraKaava = normalize(metadata.resolver) === "aurakaavaresolver"
    const isVehmaaKaava = normalize(metadata.resolver) === "vehmaakaavaresolver"
    const isLaitilaKaava = normalize(metadata.resolver) === "laitilakaavaresolver"
    const isKustaviKaava = normalize(metadata.resolver) === "kustavikaavaresolver"
    const isSieviKaava = normalize(metadata.resolver) === "sievikaavaresolver"
    const isVaalaKaava = normalize(metadata.resolver) === "vaalakaavaresolver"
    const isSiikajokiKaava = normalize(metadata.resolver) === "siikajokikaavaresolver"
    const isSiikalatvaKaava = normalize(metadata.resolver) === "siikalatvakaavaresolver"
    const isIiKaava = normalize(metadata.resolver) === "iikaavaresolver"
    const isAlavieskaKaava = normalize(metadata.resolver) === "alavieskakaavaresolver"
    const isHailuotoKaava = normalize(metadata.resolver) === "hailuotokaavaresolver"
    const isOulainenKaava = normalize(metadata.resolver) === "oulainenkaavaresolver"
    const isTaivalkoskiKaava = normalize(metadata.resolver) === "taivalkoskikaavaresolver"
    const isPoytyaKaava = normalize(metadata.resolver) === "poytyakaavaresolver"
    const isMaskuKaava = normalize(metadata.resolver) === "maskukaavaresolver"
    const isRuskoKaava = normalize(metadata.resolver) === "ruskokaavaresolver"
    const isMynamakiKaava = normalize(metadata.resolver) === "mynamakikaavaresolver"
    const isKemionsaariKaava = normalize(metadata.resolver) === "kemionsaarikaavaresolver"
    const isMarttilaKaava = normalize(metadata.resolver) === "marttilakaavaresolver"
    const isPyharantaKaava = normalize(metadata.resolver) === "pyharantakaavaresolver"
    const isTaivassaloKaava = normalize(metadata.resolver) === "taivassalokaavaresolver"
    const isKristiinankaupunkiKaava = normalize(metadata.resolver) === "kristiinankaupunkikaavaresolver"
    const isKorsnasKaava = normalize(metadata.resolver) === "korsnaskaavaresolver"
    const isKruunupyyKaava = normalize(metadata.resolver) === "kruunupyykaavaresolver"
    const isLuotoKaava = normalize(metadata.resolver) === "luotokaavaresolver"
    const isMaalahtiKaava = normalize(metadata.resolver) === "maalahtikaavaresolver"
    const isPedersoreKaava = normalize(metadata.resolver) === "pedersorekaavaresolver"
    const isUusikaarlepyyKaava = normalize(metadata.resolver) === "uusikaarlepyykaavaresolver"
    const isVoyriKaava = normalize(metadata.resolver) === "voyrikaavaresolver"
    const isPerhoKaava = normalize(metadata.resolver) === "perhokaavaresolver"
    const isLestijarviKaava = normalize(metadata.resolver) === "lestijarvikaavaresolver"
    const isIlomantsiKaava = normalize(metadata.resolver) === "ilomantsikaavaresolver"
    const isTohmajarviKaava = normalize(metadata.resolver) === "tohmajarvikaavaresolver"
    const isTammelaKaava = normalize(metadata.resolver) === "tammelakaavaresolver"
    const isLemiKaava = normalize(metadata.resolver) === "lemikaavaresolver"
    const isKangasniemiKaava = normalize(metadata.resolver) === "kangasniemikaavaresolver"
    const isKihnioKaava = normalize(metadata.resolver) === "kihniokaavaresolver"
    ...
    const isIsojokiKaava = normalize(metadata.resolver) === "isojokikaavaresolver"
    const isVimpeliKaava = normalize(metadata.resolver) === "vimpelikaavaresolver"
    ...
    const isTaipalsaariKaava = normalize(metadata.resolver) === "taipalsaarikaavaresolver"
    const isKinnulaKaava = normalize(metadata.resolver) === "kinnulakaavaresolver"
    const isNurmesKaava = normalize(metadata.resolver) === "nurmeskaavaresolver"
    const isMerijarviKaava = normalize(metadata.resolver) === "merijarvikaavaresolver"
    const isHaapajarviKaava = normalize(metadata.resolver) === "haapajarvikaavaresolver"
    const isHaapavesiKaava = normalize(metadata.resolver) === "haapavesikaavaresolver"
    const isKarsamakiKaava = normalize(metadata.resolver) === "karsamakikaavaresolver"
    const isLumijokiKaava = normalize(metadata.resolver) === "lumijokikaavaresolver"
    const isMuhosKaava = normalize(metadata.resolver) === "muhoskaavaresolver"
    const isPyhajokiKaava = normalize(metadata.resolver) === "pyhajokikaavaresolver"
    const isPyhajarviKaava = normalize(metadata.resolver) === "pyhajarvikaavaresolver"
    const isPyhantaKaava = normalize(metadata.resolver) === "pyhantakaavaresolver"
    const isPudasjarviKaava = normalize(metadata.resolver) === "pudasjarvikaavaresolver"
    const isReisjarviKaava = normalize(metadata.resolver) === "reisjarvikaavaresolver"
    const isTyrnavaKaava = normalize(metadata.resolver) === "tyrnavakaavaresolver"
    const isAsikkalaKaava = normalize(metadata.resolver) === "asikkalakaavaresolver"
    const isHartolaKaava = normalize(metadata.resolver) === "hartolakaavaresolver"
    const isKarkolaKaava = normalize(metadata.resolver) === "karkolakaavaresolver"
    const isPadasjokiKaava = normalize(metadata.resolver) === "padasjokikaavaresolver"
    const isSysmaKaava = normalize(metadata.resolver) === "sysmakaavaresolver"
    const isKoskiTlKaava = normalize(metadata.resolver) === "koskitlkaavaresolver"
    ...
    const isKeminmaaKaava = normalize(metadata.resolver) === "keminmaakaavaresolver"
    const isMuonioKaava = normalize(metadata.resolver) === "muoniokaavaresolver"
    const isPelkosenniemiKaava = normalize(metadata.resolver) === "pelkosenniemikaavaresolver"
    const isRanuaKaava = normalize(metadata.resolver) === "ranuakaavaresolver"
    ...
    const isHameenkyroKaava = normalize(metadata.resolver) === "hameenkyrokaavaresolver"
    const isIkaalinenKaava = normalize(metadata.resolver) === "ikaalinenkaavaresolver"
    const isManttaVilppulaKaava = normalize(metadata.resolver) === "manttavilppulakaavaresolver"
    const isOrivesiKaava = normalize(metadata.resolver) === "orivesikaavaresolver"
    const isPalkaneKaava = normalize(metadata.resolver) === "palkanekaavaresolver"
    const isVesilahtiKaava = normalize(metadata.resolver) === "vesilahtikaavaresolver"
    const isKaskinenKaava = normalize(metadata.resolver) === "kaskinenkaavaresolver"
    const isRuovesiKaava = normalize(metadata.resolver) === "ruovesikaavaresolver"
    const isVirratKaava = normalize(metadata.resolver) === "virratkaavaresolver"
    ...
    if (isMultiaKaava) {
      candidateIdentifiers.push({
        type: "multia_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isPetajavesiKaava) {
      candidateIdentifiers.push({
        type: "petajavesi_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isPihtipudasKaava) {
      candidateIdentifiers.push({
        type: "pihtipudas_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isUurainenKaava) {
      candidateIdentifiers.push({
        type: "uurainen_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isViitasaariKaava) {
      candidateIdentifiers.push({
        type: "viitasaari_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isIittiKaava) {
      candidateIdentifiers.push({
        type: "iitti_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isMiehikkalaKaava) {
      candidateIdentifiers.push({
        type: "miehikkala_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isSiikajokiKaava) {
      candidateIdentifiers.push({
        type: "siikajoki_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isSiikalatvaKaava) {
      candidateIdentifiers.push({
        type: "siikalatva_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isOulainenKaava) {
      candidateIdentifiers.push({
        type: "oulainen_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isTaivalkoskiKaava) {
      candidateIdentifiers.push({
        type: "taivalkoski_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isMynamakiKaava) {
      candidateIdentifiers.push({
        type: "mynamaki_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isKemionsaariKaava) {
      candidateIdentifiers.push({
        type: "kemionsaari_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isPyharantaKaava) {
      candidateIdentifiers.push({
        type: "pyharanta_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isTaivassaloKaava) {
      candidateIdentifiers.push({
        type: "taivassalo_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isKristiinankaupunkiKaava) {
      candidateIdentifiers.push({
        type: "kristiinankaupunki_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isPedersoreKaava) {
      candidateIdentifiers.push({
        type: "pedersore_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isUusikaarlepyyKaava) {
      candidateIdentifiers.push({
        type: "uusikaarlepyy_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isPerhoKaava) {
      candidateIdentifiers.push({
        type: "perho_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isLestijarviKaava) {
      candidateIdentifiers.push({
        type: "lestijarvi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isIlomantsiKaava) {
      candidateIdentifiers.push({
        type: "ilomantsi_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isTohmajarviKaava) {
      candidateIdentifiers.push({
        type: "tohmajarvi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isLemiKaava) {
      candidateIdentifiers.push({
        type: "lemi_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isKangasniemiKaava) {
      candidateIdentifiers.push({
        type: "kangasniemi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isKihnioKaava) {
      candidateIdentifiers.push({
        type: "kihnio_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isRistijarviKaava) {
      candidateIdentifiers.push({
        type: "ristijarvi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isTaipalsaariKaava) {
      candidateIdentifiers.push({
        type: "taipalsaari_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isMerijarviKaava) {
      candidateIdentifiers.push({
        type: "merijarvi_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isHaapajarviKaava) {
      candidateIdentifiers.push({
        type: "haapajarvi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isPyhantaKaava) {
      candidateIdentifiers.push({
        type: "pyhanta_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isPudasjarviKaava) {
      candidateIdentifiers.push({
        type: "pudasjarvi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isMuonioKaava) {
      candidateIdentifiers.push({
        type: "muonio_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isPelkosenniemiKaava) {
      candidateIdentifiers.push({
        type: "pelkosenniemi_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isHameenkyroKaava) {
      candidateIdentifiers.push({
        type: "hameenkyro_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    if (isIkaalinenKaava) {
      candidateIdentifiers.push({
        type: "ikaalinen_kaava_slug",
        value: metadata.slug,
      })
    }

    if (isManttaVilppulaKaava) {
      candidateIdentifiers.push({
        type: "manttavilppula_kaava_slug",
        value: metadata.slug,
      })
    }
    ...
    const region =
      resolveRegion({ metadataRegion: metadata.region, city, buyerName: metadata.developer, title: potentialProject.title }) ??
