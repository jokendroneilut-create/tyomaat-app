...
export async function resolveKotkaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Kotka")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Kotka",
    ...
    identifiers: [{ type: "kotka_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kotkan kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kotkaKaavaResolver",
