...
export async function resolvePorvooKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Porvoo")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Porvoo",
    ...
    identifiers: [{ type: "porvoo_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Porvoon kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "porvooKaavaResolver",
