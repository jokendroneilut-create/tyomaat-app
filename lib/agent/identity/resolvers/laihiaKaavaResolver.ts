...
function mapLaihiaPhase(rawPhase: string | null): string {
...
export async function resolveLaihiaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLaihiaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "laihia_kaava_slug", value: slug }],

    metadata: {
      source: "Laihian kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "laihiaKaavaResolver",
