...
function mapKauhajokiPhase(rawPhase: string | null): string {
...
export async function resolveKauhajokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKauhajokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kauhajoki_kaava_slug", value: slug }],

    metadata: {
      source: "Kauhajoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kauhajokiKaavaResolver",
