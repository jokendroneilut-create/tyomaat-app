...
function mapVirratPhase(rawPhase: string | null): string {
...
export async function resolveVirratKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVirratPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "virrat_kaava_slug", value: slug }],

    metadata: {
      source: "Virtain kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "virratKaavaResolver",
