...
function mapRiihimakiPhase(rawPhase: string | null): string {
  ...
  if (/aloitus/.test(normalized)) return PHASE_LABELS.zoning
  return PHASE_LABELS.zoning
}

export async function resolveRiihimakiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "riihimaki_kaava_slug", value: kaavaTunnus }],

    metadata: {
      source: "Riihimäen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "riihimakiKaavaResolver",
