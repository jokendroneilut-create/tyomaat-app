...
function mapSysmaPhase(rawPhase: string | null): string {
...
export async function resolveSysmaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSysmaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sysma_kaava_slug", value: slug }],

    metadata: {
      source: "Sysmän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sysmaKaavaResolver",
