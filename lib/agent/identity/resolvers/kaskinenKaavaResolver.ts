...
function mapKaskinenPhase(rawPhase: string | null): string {
...
export async function resolveKaskinenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKaskinenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kaskinen_kaava_slug", value: slug }],

    metadata: {
      source: "Kaskisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kaskinenKaavaResolver",
