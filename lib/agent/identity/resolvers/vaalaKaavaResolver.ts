...
function mapVaalaPhase(rawPhase: string | null): string {
...
export async function resolveVaalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVaalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "vaala_kaava_slug", value: slug }],

    metadata: {
      source: "Vaalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vaalaKaavaResolver",
