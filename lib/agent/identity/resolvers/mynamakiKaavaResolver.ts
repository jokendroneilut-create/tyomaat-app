...
function mapMynamakiPhase(rawPhase: string | null): string {
...
export async function resolveMynamakiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMynamakiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "mynamaki_kaava_slug", value: slug }],

    metadata: {
      source: "Mynämäen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "mynamakiKaavaResolver",
