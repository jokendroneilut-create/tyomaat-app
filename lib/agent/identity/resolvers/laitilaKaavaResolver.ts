...
function mapLaitilaPhase(rawPhase: string | null): string {
...
export async function resolveLaitilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLaitilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "laitila_kaava_slug", value: slug }],

    metadata: {
      source: "Laitilan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "laitilaKaavaResolver",
