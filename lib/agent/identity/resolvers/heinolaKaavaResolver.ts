...
function mapHeinolaPhase(rawPhase: string | null): string {
...
export async function resolveHeinolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHeinolaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: kaavaTunnus ? [{ type: "heinola_kaava_tunnus", value: kaavaTunnus }] : [],

    metadata: {
      source: "Heinolan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "heinolaKaavaResolver",
