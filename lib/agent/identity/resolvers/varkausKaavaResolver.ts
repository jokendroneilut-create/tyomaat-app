...
function mapVarkausPhase(rawPhase: string | null): string {
...
export async function resolveVarkausKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVarkausPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "varkaus_kaava_slug", value: slug }],

    metadata: {
      source: "Varkauden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "varkausKaavaResolver",
