...
function mapKeitelePhase(rawPhase: string | null): string {
...
export async function resolveKeiteleKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKeitelePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "keitele_kaava_slug", value: slug }],

    metadata: {
      source: "Keiteleen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "keiteleKaavaResolver",
