...
function mapIiPhase(rawPhase: string | null): string {
...
export async function resolveIiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ii_kaava_slug", value: slug }],

    metadata: {
      source: "Iin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "iiKaavaResolver",
