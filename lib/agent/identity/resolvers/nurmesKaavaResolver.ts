...
function mapNurmesPhase(rawPhase: string | null): string {
...
export async function resolveNurmesKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapNurmesPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "nurmes_kaava_slug", value: slug }],

    metadata: {
      source: "Nurmeksen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "nurmesKaavaResolver",
