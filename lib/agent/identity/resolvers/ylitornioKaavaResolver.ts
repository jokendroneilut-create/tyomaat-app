...
function mapYlitornioPhase(rawPhase: string | null): string {
...
export async function resolveYlitornioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapYlitornioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ylitornio_kaava_slug", value: slug }],

    metadata: {
      source: "Ylitornion kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ylitornioKaavaResolver",
