...
function mapHirvensalmiPhase(rawPhase: string | null): string {
...
export async function resolveHirvensalmiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHirvensalmiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hirvensalmi_kaava_slug", value: slug }],

    metadata: {
      source: "Hirvensalmen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hirvensalmiKaavaResolver",
