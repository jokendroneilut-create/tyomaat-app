...
function mapSiuntioPhase(rawPhase: string | null): string {
...
export async function resolveSiuntioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSiuntioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "siuntio_kaava_slug", value: slug }],

    metadata: {
      source: "Siuntion kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "siuntioKaavaResolver",
