...
function mapMaalahtiPhase(rawPhase: string | null): string {
...
export async function resolveMaalahtiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMaalahtiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "maalahti_kaava_slug", value: slug }],

    metadata: {
      source: "Maalahden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "maalahtiKaavaResolver",
