...
function mapMultiaPhase(rawPhase: string | null): string {
...
export async function resolveMultiaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMultiaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "multia_kaava_slug", value: slug }],

    metadata: {
      source: "Multian kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "multiaKaavaResolver",
