...
function mapParainenPhase(rawPhase: string | null): string {
...
export async function resolveParainenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapParainenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "parainen_kaava_slug", value: slug }],

    metadata: {
      source: "Paraisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "parainenKaavaResolver",
