...
export async function resolveTornioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "tornio_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Tornion kaavatori",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "tornioKaavaResolver",
