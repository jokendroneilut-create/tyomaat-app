...
function mapKristiinankaupunkiPhase(rawPhase: string | null): string {
...
export async function resolveKristiinankaupunkiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKristiinankaupunkiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kristiinankaupunki_kaava_slug", value: slug }],

    metadata: {
      source: "Kristiinankaupungin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kristiinankaupunkiKaavaResolver",
