...
function mapPaimioPhase(rawPhase: string | null): string {
...
export async function resolvePaimioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPaimioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "paimio_kaava_slug", value: slug }],

    metadata: {
      source: "Paimion kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "paimioKaavaResolver",
