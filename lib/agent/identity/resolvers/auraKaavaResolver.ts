...
function mapAuraPhase(rawPhase: string | null): string {
...
export async function resolveAuraKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAuraPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "aura_kaava_slug", value: slug }],

    metadata: {
      source: "Auran kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "auraKaavaResolver",
