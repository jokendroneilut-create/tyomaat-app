...
function mapPunkalaidunPhase(rawPhase: string | null): string {
...
export async function resolvePunkalaidunKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPunkalaidunPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "punkalaidun_kaava_slug", value: slug }],

    metadata: {
      source: "Punkalaitumen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "punkalaidunKaavaResolver",
