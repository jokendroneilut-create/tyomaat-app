...
function mapKangasalaPhase(rawPhase: string | null): string {
  ...
  if (/luonnos|selostus/.test(normalized)) return PHASE_LABELS.planning
  if (/osallistumis/.test(normalized)) return PHASE_LABELS.zoning
  return PHASE_LABELS.zoning
}

export async function resolveKangasalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKangasalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kangasala_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kangasalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kangasalaKaavaResolver",
