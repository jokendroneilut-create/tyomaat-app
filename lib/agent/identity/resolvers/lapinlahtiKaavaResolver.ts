...
function mapLapinlahtiPhase(rawPhase: string | null): string {
...
export async function resolveLapinlahtiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLapinlahtiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lapinlahti_kaava_slug", value: slug }],

    metadata: {
      source: "Lapinlahden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lapinlahtiKaavaResolver",
