...
function mapPuumalaPhase(rawPhase: string | null): string {
...
export async function resolvePuumalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPuumalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "puumala_kaava_slug", value: slug }],

    metadata: {
      source: "Puumalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "puumalaKaavaResolver",
