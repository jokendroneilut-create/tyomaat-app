...
function mapJoutsaPhase(rawPhase: string | null): string {
...
export async function resolveJoutsaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapJoutsaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "joutsa_kaava_slug", value: slug }],

    metadata: {
      source: "Joutsan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "joutsaKaavaResolver",
