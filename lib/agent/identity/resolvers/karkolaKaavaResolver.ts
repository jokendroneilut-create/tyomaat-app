...
function mapKarkolaPhase(rawPhase: string | null): string {
...
export async function resolveKarkolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKarkolaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "karkola_kaava_slug", value: slug }],

    metadata: {
      source: "Kärkölän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "karkolaKaavaResolver",
