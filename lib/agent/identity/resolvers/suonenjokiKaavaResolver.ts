...
function mapSuonenjokiPhase(rawPhase: string | null): string {
...
export async function resolveSuonenjokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSuonenjokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "suonenjoki_kaava_slug", value: slug }],

    metadata: {
      source: "Suonenjoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "suonenjokiKaavaResolver",
