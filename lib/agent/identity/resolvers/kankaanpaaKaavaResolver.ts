...
function mapKankaanpaaPhase(rawPhase: string | null): string {
...
export async function resolveKankaanpaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKankaanpaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kankaanpaa_kaava_slug", value: slug }],

    metadata: {
      source: "Kankaanpään kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kankaanpaaKaavaResolver",
