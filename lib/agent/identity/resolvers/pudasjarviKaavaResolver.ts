...
function mapPudasjarviPhase(rawPhase: string | null): string {
...
export async function resolvePudasjarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPudasjarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pudasjarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Pudasjärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pudasjarviKaavaResolver",
