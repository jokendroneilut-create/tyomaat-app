...
function mapSavonlinnaPhase(rawPhase: string | null): string {
...
export async function resolveSavonlinnaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSavonlinnaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "savonlinna_kaava_title", value: operation }],

    metadata: {
      source: "Savonlinnan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "savonlinnaKaavaResolver",
