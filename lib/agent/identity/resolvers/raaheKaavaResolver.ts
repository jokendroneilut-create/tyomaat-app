...
function mapRaahePhase(rawPhase: string | null): string {
...
export async function resolveRaaheKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRaahePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "raahe_kaava_id", value: kaavaTunnus }],

    metadata: {
      source: "Raahen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "raaheKaavaResolver",
