...
function mapLimingaPhase(rawPhase: string | null): string {
...
export async function resolveLimingaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLimingaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "liminka_kaava_slug", value: slug }],

    metadata: {
      source: "Limingan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "limingaKaavaResolver",
