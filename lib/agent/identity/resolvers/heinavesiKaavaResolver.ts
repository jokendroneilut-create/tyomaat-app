...
function mapHeinavesiPhase(rawPhase: string | null): string {
...
export async function resolveHeinavesiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHeinavesiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "heinavesi_kaava_slug", value: slug }],

    metadata: {
      source: "Heinäveden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "heinavesiKaavaResolver",
