...
function mapAhtariPhase(rawPhase: string | null): string {
...
export async function resolveAhtariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAhtariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ahtari_kaava_slug", value: slug }],

    metadata: {
      source: "Ähtärin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ahtariKaavaResolver",
