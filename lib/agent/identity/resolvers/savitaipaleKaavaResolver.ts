...
function mapSavitaipalePhase(rawPhase: string | null): string {
...
export async function resolveSavitaipaleKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSavitaipalePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "savitaipale_kaava_slug", value: slug }],

    metadata: {
      source: "Savitaipaleen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "savitaipaleKaavaResolver",
