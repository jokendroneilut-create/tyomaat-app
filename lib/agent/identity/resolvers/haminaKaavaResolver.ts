...
function mapHaminaPhase(rawPhase: string | null): string {
...
export async function resolveHaminaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHaminaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hamina_kaava_tunnus", value: kaavaTunnus ?? slug }],

    metadata: {
      source: "Haminan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "haminaKaavaResolver",
