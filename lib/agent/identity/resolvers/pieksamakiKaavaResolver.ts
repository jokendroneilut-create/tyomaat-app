...
function mapPieksamakiPhase(rawPhase: string | null): string {
...
export async function resolvePieksamakiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPieksamakiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pieksamaki_kaava_slug", value: slug }],

    metadata: {
      source: "Pieksämäen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pieksamakiKaavaResolver",
