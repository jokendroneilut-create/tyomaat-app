...
function mapKiuruvesiPhase(rawPhase: string | null): string {
...
export async function resolveKiuruvesiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKiuruvesiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kiuruvesi_kaava_slug", value: slug }],

    metadata: {
      source: "Kiuruveden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kiuruvesiKaavaResolver",
