...
function mapRanuaPhase(rawPhase: string | null): string {
...
export async function resolveRanuaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRanuaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ranua_kaava_slug", value: slug }],

    metadata: {
      source: "Ranuan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ranuaKaavaResolver",
