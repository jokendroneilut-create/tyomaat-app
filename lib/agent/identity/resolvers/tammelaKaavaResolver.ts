...
function mapTammelaPhase(rawPhase: string | null): string {
...
export async function resolveTammelaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTammelaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "tammela_kaava_slug", value: slug }],

    metadata: {
      source: "Tammelan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "tammelaKaavaResolver",
