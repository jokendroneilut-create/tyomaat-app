...
function mapEnonkoskiPhase(rawPhase: string | null): string {
...
export async function resolveEnonkoskiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapEnonkoskiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "enonkoski_kaava_slug", value: slug }],

    metadata: {
      source: "Enonkosken kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "enonkoskiKaavaResolver",
