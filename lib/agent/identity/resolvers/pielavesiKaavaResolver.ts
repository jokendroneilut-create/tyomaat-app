...
function mapPielavesiPhase(rawPhase: string | null): string {
...
export async function resolvePielavesiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPielavesiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pielavesi_kaava_slug", value: slug }],

    metadata: {
      source: "Pielaveden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pielavesiKaavaResolver",
