...
function mapSiikajokiPhase(rawPhase: string | null): string {
...
export async function resolveSiikajokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSiikajokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "siikajoki_kaava_slug", value: slug }],

    metadata: {
      source: "Siikajoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "siikajokiKaavaResolver",
