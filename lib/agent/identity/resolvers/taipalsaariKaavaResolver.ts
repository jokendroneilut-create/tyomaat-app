...
function mapTaipalsaariPhase(rawPhase: string | null): string {
...
export async function resolveTaipalsaariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTaipalsaariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "taipalsaari_kaava_slug", value: slug }],

    metadata: {
      source: "Taipalsaaren kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "taipalsaariKaavaResolver",
