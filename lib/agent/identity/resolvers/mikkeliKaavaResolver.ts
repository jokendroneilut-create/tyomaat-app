...
export async function resolveMikkeliKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Mikkeli")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Mikkeli",
    ...
    identifiers: [{ type: "mikkeli_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Mikkelin kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "mikkeliKaavaResolver",
