...
function mapKolariPhase(rawPhase: string | null): string {
...
export async function resolveKolariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKolariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kolari_kaava_slug", value: slug }],

    metadata: {
      source: "Kolarin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kolariKaavaResolver",
