...
export async function resolveOuluKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Oulu")
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "oulu_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Oulun kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ouluKaavaResolver",
