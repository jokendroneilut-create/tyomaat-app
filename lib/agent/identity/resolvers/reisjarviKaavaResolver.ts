...
function mapReisjarviPhase(rawPhase: string | null): string {
...
export async function resolveReisjarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapReisjarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "reisjarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Reisjärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "reisjarviKaavaResolver",
