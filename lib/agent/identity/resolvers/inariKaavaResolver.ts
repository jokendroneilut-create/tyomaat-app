...
function mapInariPhase(rawPhase: string | null): string {
...
export async function resolveInariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapInariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "inari_kaava_slug", value: slug }],

    metadata: {
      source: "Inarin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "inariKaavaResolver",
