...
function mapUtsjokiPhase(rawPhase: string | null): string {
...
export async function resolveUtsjokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapUtsjokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "utsjoki_kaava_slug", value: slug }],

    metadata: {
      source: "Utsjoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "utsjokiKaavaResolver",
