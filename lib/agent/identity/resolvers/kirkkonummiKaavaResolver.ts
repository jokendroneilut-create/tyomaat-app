...
export async function resolveKirkkonummiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Kirkkonummi")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Kirkkonummi",
    ...
    identifiers: [{ type: "kirkkonummi_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kirkkonummen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kirkkonummiKaavaResolver",
