...
function mapMuuramePhase(rawPhase: string | null): string {
...
export async function resolveMuurameKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMuuramePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "muurame_kaava_slug", value: slug }],

    metadata: {
      source: "Muuramen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "muurameKaavaResolver",
