...
function mapKontiolahtiPhase(rawPhase: string | null): string {
...
export async function resolveKontiolahtiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKontiolahtiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kontiolahti_kaava_slug", value: slug }],

    metadata: {
      source: "Kontiolahden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kontiolahtiKaavaResolver",
