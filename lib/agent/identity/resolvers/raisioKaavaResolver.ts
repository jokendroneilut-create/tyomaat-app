...
function mapRaisioPhase(rawPhase: string | null): string {
  const normalized = (rawPhase ?? "").toLowerCase()
  if (/lainvoima|voimaantulo/.test(normalized)) return PHASE_LABELS.completed
...
export async function resolveRaisioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const slug = findFact(facts, "kaava_tunnus")?.fact_value ?? null
  ...
  const completed = /lainvoima|voimaantulo/i.test(phase ?? "")
  const phaseHint = mapRaisioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "raisio_kaava_slug", value: slug }],

    metadata: {
      source: "Raision kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "raisioKaavaResolver",
