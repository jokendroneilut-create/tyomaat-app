...
function mapVehmaaPhase(rawPhase: string | null): string {
...
export async function resolveVehmaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVehmaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "vehmaa_kaava_slug", value: slug }],

    metadata: {
      source: "Vehmaan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vehmaaKaavaResolver",
