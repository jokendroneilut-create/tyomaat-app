...
function mapKuhmoPhase(rawPhase: string | null): string {
...
export async function resolveKuhmoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKuhmoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kuhmo_kaava_slug", value: slug }],

    metadata: {
      source: "Kuhmon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kuhmoKaavaResolver",
