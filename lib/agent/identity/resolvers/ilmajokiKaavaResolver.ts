...
function mapIlmajokiPhase(rawPhase: string | null): string {
...
export async function resolveIlmajokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIlmajokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ilmajoki_kaava_slug", value: slug }],

    metadata: {
      source: "Ilmajoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ilmajokiKaavaResolver",
