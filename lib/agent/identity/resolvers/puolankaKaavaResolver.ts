...
function mapPuolankaPhase(rawPhase: string | null): string {
...
export async function resolvePuolankaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPuolankaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "puolanka_kaava_slug", value: slug }],

    metadata: {
      source: "Puolangan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "puolankaKaavaResolver",
