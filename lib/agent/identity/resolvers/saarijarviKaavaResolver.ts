...
function mapSaarijarviPhase(rawPhase: string | null): string {
...
export async function resolveSaarijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSaarijarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "saarijarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Saarijärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "saarijarviKaavaResolver",
