...
export async function resolveKouvolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Kouvola")
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kouvola_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kouvolan kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kouvolaKaavaResolver",
