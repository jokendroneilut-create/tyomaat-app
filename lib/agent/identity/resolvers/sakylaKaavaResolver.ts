...
function mapSakylaPhase(rawPhase: string | null): string {
...
export async function resolveSakylaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSakylaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sakyla_kaava_slug", value: slug }],

    metadata: {
      source: "Säkylän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sakylaKaavaResolver",
