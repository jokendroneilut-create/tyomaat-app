...
function mapHailuotoPhase(rawPhase: string | null): string {
...
export async function resolveHailuotoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHailuotoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hailuoto_kaava_slug", value: slug }],

    metadata: {
      source: "Hailuodon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hailuotoKaavaResolver",
