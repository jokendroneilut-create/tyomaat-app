...
function mapHankoPhase(rawPhase: string | null): string {
...
export async function resolveHankoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHankoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hanko_kaava_slug", value: slug }],

    metadata: {
      source: "Hangon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hankoKaavaResolver",
