...
export async function resolveMantsalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMantsalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "mantsala_kaava_slug", value: slug }],

    metadata: {
      source: "Mäntsälän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "mantsalaKaavaResolver",
