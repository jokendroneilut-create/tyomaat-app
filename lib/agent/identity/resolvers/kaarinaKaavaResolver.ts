...
function mapKaarinaPhase(rawPhase: string | null): string {
  const normalized = (rawPhase ?? "").toLowerCase()
  if (/lainvoimainen|voimaantulo/.test(normalized)) return PHASE_LABELS.completed
...
export async function resolveKaarinaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const completed = /lainvoimainen|voimaantulo/i.test(phase ?? "")
  const phaseHint = mapKaarinaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kaarina_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kaarinan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kaarinaKaavaResolver",
