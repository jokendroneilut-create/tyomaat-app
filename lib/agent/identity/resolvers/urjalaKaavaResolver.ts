...
function mapUrjalaPhase(rawPhase: string | null): string {
...
export async function resolveUrjalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapUrjalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "urjala_kaava_slug", value: slug }],

    metadata: {
      source: "Urjalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "urjalaKaavaResolver",
