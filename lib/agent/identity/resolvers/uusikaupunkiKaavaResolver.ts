...
function mapUusikaupunkiPhase(rawPhase: string | null): string {
...
export async function resolveUusikaupunkiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapUusikaupunkiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "uusikaupunki_kaava_slug", value: slug }],

    metadata: {
      source: "Uudenkaupungin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "uusikaupunkiKaavaResolver",
