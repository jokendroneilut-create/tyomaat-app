...
function mapJamijarviPhase(rawPhase: string | null): string {
...
export async function resolveJamijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapJamijarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "jamijarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Jämijärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "jamijarviKaavaResolver",
