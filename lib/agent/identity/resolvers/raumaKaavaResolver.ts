...
function mapRaumaPhase(rawPhase: string | null): string {
...
export async function resolveRaumaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRaumaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "rauma_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Rauman kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "raumaKaavaResolver",
