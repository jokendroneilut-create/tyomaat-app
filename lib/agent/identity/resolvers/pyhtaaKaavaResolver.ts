...
function mapPyhtaaPhase(rawPhase: string | null): string {
...
export async function resolvePyhtaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPyhtaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pyhtaa_kaava_slug", value: slug }],

    metadata: {
      source: "Pyhtään kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pyhtaaKaavaResolver",
