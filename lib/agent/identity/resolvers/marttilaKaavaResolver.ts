...
function mapMarttilaPhase(rawPhase: string | null): string {
...
export async function resolveMarttilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMarttilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "marttila_kaava_slug", value: slug }],

    metadata: {
      source: "Marttilan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "marttilaKaavaResolver",
