...
function mapAlavieskaPhase(rawPhase: string | null): string {
...
export async function resolveAlavieskaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAlavieskaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "alavieska_kaava_slug", value: slug }],

    metadata: {
      source: "Alavieskan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "alavieskaKaavaResolver",
