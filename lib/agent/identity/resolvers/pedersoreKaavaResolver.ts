...
function mapPedersorePhase(rawPhase: string | null): string {
...
export async function resolvePedersoreKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPedersorePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pedersore_kaava_slug", value: slug }],

    metadata: {
      source: "Pedersören kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pedersoreKaavaResolver",
