...
function mapPadasjokiPhase(rawPhase: string | null): string {
...
export async function resolvePadasjokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPadasjokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "padasjoki_kaava_slug", value: slug }],

    metadata: {
      source: "Padasjoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "padasjokiKaavaResolver",
