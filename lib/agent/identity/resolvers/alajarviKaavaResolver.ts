...
function mapAlajarviPhase(rawPhase: string | null): string {
...
export async function resolveAlajarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAlajarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "alajarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Alajärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "alajarviKaavaResolver",
