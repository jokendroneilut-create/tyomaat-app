...
function mapOrimattilaPhase(rawPhase: string | null): string {
...
export async function resolveOrimattilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapOrimattilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "orimattila_kaava_slug", value: slug }],

    metadata: {
      source: "Orimattilan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "orimattilaKaavaResolver",
