...
function mapVeteliPhase(rawPhase: string | null): string {
...
export async function resolveVeteliKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVeteliPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "veteli_kaava_slug", value: slug }],

    metadata: {
      source: "Vetelin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "veteliKaavaResolver",
