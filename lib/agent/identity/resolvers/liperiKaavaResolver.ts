...
function mapLiperiPhase(rawPhase: string | null): string {
...
export async function resolveLiperiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLiperiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "liperi_kaava_slug", value: slug }],

    metadata: {
      source: "Liperin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "liperiKaavaResolver",
