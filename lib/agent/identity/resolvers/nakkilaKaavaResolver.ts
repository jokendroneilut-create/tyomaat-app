...
function mapNakkilaPhase(rawPhase: string | null): string {
...
export async function resolveNakkilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapNakkilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "nakkila_kaava_slug", value: slug }],

    metadata: {
      source: "Nakkilan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "nakkilaKaavaResolver",
