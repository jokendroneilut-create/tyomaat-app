...
function mapUlvilaPhase(rawPhase: string | null): string {
...
export async function resolveUlvilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapUlvilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ulvila_kaava_slug", value: slug }],

    metadata: {
      source: "Ulvilan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ulvilaKaavaResolver",
