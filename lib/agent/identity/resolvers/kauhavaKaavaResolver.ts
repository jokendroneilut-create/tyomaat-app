...
function mapKauhavaPhase(rawPhase: string | null): string {
...
export async function resolveKauhavaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKauhavaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kauhava_kaava_slug", value: slug }],

    metadata: {
      source: "Kauhavan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kauhavaKaavaResolver",
