...
export async function resolveRaaseporiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const kaavaId = findFact(facts, "kaava_tunnus")?.fact_value ?? null
  ...
  const phaseHint = mapRaaseporiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "raasepori_kaava_id", value: kaavaId }],

    metadata: {
      source: "Raaseporin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "raaseporiKaavaResolver",

      operation,
      region: "Uusimaa",
      kaava_tunnus: kaavaId,
