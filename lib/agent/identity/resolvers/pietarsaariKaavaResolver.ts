...
function mapPietarsaariPhase(rawPhase: string | null): string {
...
export async function resolvePietarsaariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPietarsaariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pietarsaari_kaava_slug", value: slug }],

    metadata: {
      source: "Pietarsaaren kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pietarsaariKaavaResolver",
