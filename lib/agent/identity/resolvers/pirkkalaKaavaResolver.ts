...
function mapPirkkalaPhase(rawPhase: string | null): string {
...
export async function resolvePirkkalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPirkkalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pirkkala_kaava_id", value: kaavaTunnus }],

    metadata: {
      source: "Pirkkalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pirkkalaKaavaResolver",
