...
function mapSiikalatvaPhase(rawPhase: string | null): string {
...
export async function resolveSiikalatvaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSiikalatvaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "siikalatva_kaava_slug", value: slug }],

    metadata: {
      source: "Siikalatvan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "siikalatvaKaavaResolver",
