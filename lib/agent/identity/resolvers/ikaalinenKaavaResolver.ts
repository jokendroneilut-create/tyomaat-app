...
function mapIkaalinenPhase(rawPhase: string | null): string {
...
export async function resolveIkaalinenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIkaalinenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ikaalinen_kaava_slug", value: slug }],

    metadata: {
      source: "Ikaalisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ikaalinenKaavaResolver",
