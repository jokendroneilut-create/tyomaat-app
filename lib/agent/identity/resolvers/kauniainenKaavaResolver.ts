...
function mapKauniainenPhase(rawPhase: string | null): string {
...
export async function resolveKauniainenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKauniainenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kauniainen_kaava_slug", value: slug }],

    metadata: {
      source: "Kauniaisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kauniainenKaavaResolver",
