...
function mapPelkosenniemiPhase(rawPhase: string | null): string {
...
export async function resolvePelkosenniemiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPelkosenniemiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pelkosenniemi_kaava_slug", value: slug }],

    metadata: {
      source: "Pelkosenniemen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pelkosenniemiKaavaResolver",
