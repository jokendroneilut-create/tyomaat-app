...
function mapLietoPhase(rawPhase: string | null): string {
...
export async function resolveLietoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLietoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lieto_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Liedon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lietoKaavaResolver",
