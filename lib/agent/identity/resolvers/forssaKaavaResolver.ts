...
function mapForssaPhase(rawPhase: string | null): string {
...
export async function resolveForssaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapForssaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "forssa_kaava_slug", value: slug }],

    metadata: {
      source: "Forssan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "forssaKaavaResolver",
