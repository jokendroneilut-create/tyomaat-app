...
function mapKruunupyyPhase(rawPhase: string | null): string {
...
export async function resolveKruunupyyKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKruunupyyPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kruunupyy_kaava_slug", value: slug }],

    metadata: {
      source: "Kruunupyyn kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kruunupyyKaavaResolver",
