...
function mapVirolahtiPhase(rawPhase: string | null): string {
...
export async function resolveVirolahtiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVirolahtiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "virolahti_kaava_slug", value: slug }],

    metadata: {
      source: "Virolahden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "virolahtiKaavaResolver",
