...
function mapUusikaarlepyyPhase(rawPhase: string | null): string {
...
export async function resolveUusikaarlepyyKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapUusikaarlepyyPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "uusikaarlepyy_kaava_slug", value: slug }],

    metadata: {
      source: "Uusikaarlepyyn kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "uusikaarlepyyKaavaResolver",
