...
function mapSomeroPhase(rawPhase: string | null): string {
...
export async function resolveSomeroKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSomeroPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "somero_kaava_slug", value: slug }],

    metadata: {
      source: "Someron kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "someroKaavaResolver",
