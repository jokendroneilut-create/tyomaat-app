...
function mapTuusniemiPhase(rawPhase: string | null): string {
...
export async function resolveTuusniemiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTuusniemiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "tuusniemi_kaava_slug", value: slug }],

    metadata: {
      source: "Tuusniemen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "tuusniemiKaavaResolver",
