...
function mapIittiPhase(rawPhase: string | null): string {
...
export async function resolveIittiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIittiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "iitti_kaava_slug", value: slug }],

    metadata: {
      source: "Iitin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "iittiKaavaResolver",
