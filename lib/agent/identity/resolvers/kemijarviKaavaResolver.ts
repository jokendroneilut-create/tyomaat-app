...
function mapKemijarviPhase(rawPhase: string | null): string {
...
export async function resolveKemijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKemijarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kemijarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Kemijärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kemijarviKaavaResolver",
