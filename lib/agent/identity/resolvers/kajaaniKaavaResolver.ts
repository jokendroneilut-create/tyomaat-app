...
function mapKajaaniPhase(rawPhase: string | null): string {
  ...
  if (/tiedoksianto/.test(normalized)) return PHASE_LABELS.permit
...
export async function resolveKajaaniKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kajaani_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kajaanin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kajaaniKaavaResolver",
