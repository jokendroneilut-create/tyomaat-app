...
function mapUurainenPhase(rawPhase: string | null): string {
...
export async function resolveUurainenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapUurainenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "uurainen_kaava_slug", value: slug }],

    metadata: {
      source: "Uuraisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "uurainenKaavaResolver",
