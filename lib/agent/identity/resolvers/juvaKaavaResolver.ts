...
function mapJuvaPhase(rawPhase: string | null): string {
...
export async function resolveJuvaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapJuvaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "juva_kaava_slug", value: slug }],

    metadata: {
      source: "Juvan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "juvaKaavaResolver",
