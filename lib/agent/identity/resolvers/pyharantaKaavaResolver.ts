...
function mapPyharantaPhase(rawPhase: string | null): string {
...
export async function resolvePyharantaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPyharantaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pyharanta_kaava_slug", value: slug }],

    metadata: {
      source: "Pyhärannan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pyharantaKaavaResolver",
