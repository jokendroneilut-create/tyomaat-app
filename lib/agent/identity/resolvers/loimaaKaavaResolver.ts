...
function mapLoimaaPhase(rawPhase: string | null): string {
...
export async function resolveLoimaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLoimaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "loimaa_kaava_slug", value: slug }],

    metadata: {
      source: "Loimaan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "loimaaKaavaResolver",
