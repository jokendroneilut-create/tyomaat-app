...
function mapPihtipudasPhase(rawPhase: string | null): string {
...
export async function resolvePihtipudasKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPihtipudasPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pihtipudas_kaava_slug", value: slug }],

    metadata: {
      source: "Pihtiputaan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pihtipudasKaavaResolver",
