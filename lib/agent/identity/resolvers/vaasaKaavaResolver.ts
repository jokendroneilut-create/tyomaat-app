...
export async function resolveVaasaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Vaasa")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Vaasa",
    ...
    identifiers: [{ type: "vaasa_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Vaasan kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vaasaKaavaResolver",
