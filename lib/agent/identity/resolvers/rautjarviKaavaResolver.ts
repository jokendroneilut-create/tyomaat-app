...
function mapRautjarviPhase(rawPhase: string | null): string {
...
export async function resolveRautjarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRautjarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "rautjarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Rautjärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "rautjarviKaavaResolver",
