...
function mapHollolaPhase(rawPhase: string | null): string {
...
export async function resolveHollolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHollolaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hollola_kaava_id", value: kaavaTunnus }],

    metadata: {
      source: "Hollolan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hollolaKaavaResolver",
