...
function mapLuotoPhase(rawPhase: string | null): string {
...
export async function resolveLuotoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLuotoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "luoto_kaava_slug", value: slug }],

    metadata: {
      source: "Luodon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "luotoKaavaResolver",
