...
function mapKeminmaaPhase(rawPhase: string | null): string {
...
export async function resolveKeminmaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKeminmaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "keminmaa_kaava_slug", value: slug }],

    metadata: {
      source: "Keminmaan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "keminmaaKaavaResolver",
