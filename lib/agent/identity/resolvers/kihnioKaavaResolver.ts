...
function mapKihnioPhase(rawPhase: string | null): string {
...
export async function resolveKihnioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKihnioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kihnio_kaava_slug", value: slug }],

    metadata: {
      source: "Kihniön tuulivoimahankkeet",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kihnioKaavaResolver",
