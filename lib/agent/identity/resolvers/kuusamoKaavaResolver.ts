...
function mapKuusamoPhase(rawPhase: string | null): string {
...
export async function resolveKuusamoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKuusamoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kuusamo_kaava_slug", value: slug }],

    metadata: {
      source: "Kuusamon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kuusamoKaavaResolver",
