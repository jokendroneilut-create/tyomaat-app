...
function mapAanekoskiPhase(rawPhase: string | null): string {
...
export async function resolveAanekoskiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAanekoskiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: kaavaTunnus ? [{ type: "aanekoski_kaava_tunnus", value: kaavaTunnus }] : [],

    metadata: {
      source: "Äänekosken kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "aanekoskiKaavaResolver",
