...
function mapPerhoPhase(rawPhase: string | null): string {
...
export async function resolvePerhoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPerhoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "perho_kaava_slug", value: slug }],

    metadata: {
      source: "Perhon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "perhoKaavaResolver",
