...
/*
 * "Käsittelyvaiheet"-kenttä on vapaamuotoinen kronologinen kertomus, ei
 * strukturoitu vaihelista, joten poimitaan edistynein mainittu vaihe
 * tekstistä prioriteettijärjestyksessä sen sijaan, että luotettaisiin
 * tekstin järjestykseen.
 */
function mapImatraPhase(rawPhase: string | null): string {
...
export async function resolveImatraKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapImatraPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "imatra_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Imatran kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "imatraKaavaResolver",
