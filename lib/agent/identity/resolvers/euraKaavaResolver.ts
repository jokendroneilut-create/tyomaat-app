...
function mapEuraPhase(rawPhase: string | null): string {
...
export async function resolveEuraKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapEuraPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "eura_kaava_slug", value: slug }],

    metadata: {
      source: "Euran kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "euraKaavaResolver",
