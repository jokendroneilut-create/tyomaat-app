...
function mapOrivesiPhase(rawPhase: string | null): string {
...
export async function resolveOrivesiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapOrivesiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "orivesi_kaava_slug", value: slug }],

    metadata: {
      source: "Oriveden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "orivesiKaavaResolver",
