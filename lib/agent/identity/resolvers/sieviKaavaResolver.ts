...
function mapSieviPhase(rawPhase: string | null): string {
...
export async function resolveSieviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSieviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sievi_kaava_slug", value: slug }],

    metadata: {
      source: "Sievin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sieviKaavaResolver",
