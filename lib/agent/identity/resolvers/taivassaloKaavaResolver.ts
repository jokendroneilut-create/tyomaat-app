...
function mapTaivassaloPhase(rawPhase: string | null): string {
...
export async function resolveTaivassaloKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTaivassaloPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "taivassalo_kaava_slug", value: slug }],

    metadata: {
      source: "Taivassalon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "taivassaloKaavaResolver",
