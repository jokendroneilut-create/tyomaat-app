...
function mapKangasniemiPhase(rawPhase: string | null): string {
...
export async function resolveKangasniemiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKangasniemiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kangasniemi_kaava_slug", value: slug }],

    metadata: {
      source: "Kangasniemen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kangasniemiKaavaResolver",
