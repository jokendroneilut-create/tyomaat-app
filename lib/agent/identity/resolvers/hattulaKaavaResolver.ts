...
function mapHattulaPhase(rawPhase: string | null): string {
...
export async function resolveHattulaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHattulaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hattula_kaava_slug", value: slug }],

    metadata: {
      source: "Hattulan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hattulaKaavaResolver",
