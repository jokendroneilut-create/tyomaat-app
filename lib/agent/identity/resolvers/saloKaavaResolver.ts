...
export async function resolveSaloKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Salo",
    ...
    identifiers: [{ type: "salo_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Salon kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "saloKaavaResolver",
