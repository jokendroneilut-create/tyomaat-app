...
export async function resolveSeinajokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Seinäjoki")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Seinäjoki",
    ...
    identifiers: [{ type: "seinajoki_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Seinäjoen kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "seinajokiKaavaResolver",
