...
function mapJokioinenPhase(rawPhase: string | null): string {
...
export async function resolveJokioinenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapJokioinenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "jokioinen_kaava_slug", value: slug }],

    metadata: {
      source: "Jokioisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "jokioinenKaavaResolver",
