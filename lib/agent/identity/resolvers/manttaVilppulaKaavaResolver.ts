...
function mapManttaVilppulaPhase(rawPhase: string | null): string {
...
export async function resolveManttaVilppulaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapManttaVilppulaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "manttavilppula_kaava_slug", value: slug }],

    metadata: {
      source: "Mänttä-Vilppulan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "manttaVilppulaKaavaResolver",
