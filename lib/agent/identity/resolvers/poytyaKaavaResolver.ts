...
function mapPoytyaPhase(rawPhase: string | null): string {
...
export async function resolvePoytyaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPoytyaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "poytya_kaava_slug", value: slug }],

    metadata: {
      source: "Pöytyän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "poytyaKaavaResolver",
