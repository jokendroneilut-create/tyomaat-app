...
function mapKoskiTlPhase(rawPhase: string | null): string {
...
export async function resolveKoskiTlKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKoskiTlPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "koski_tl_kaava_slug", value: slug }],

    metadata: {
      source: "Kosken Tl kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "koskiTlKaavaResolver",
