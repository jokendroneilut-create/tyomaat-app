...
function mapSiilinjarviPhase(rawPhase: string | null): string {
...
export async function resolveSiilinjarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSiilinjarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "siilinjarvi_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Siilinjärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "siilinjarviKaavaResolver",
