...
function mapPaltamoPhase(rawPhase: string | null): string {
...
export async function resolvePaltamoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPaltamoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "paltamo_kaava_slug", value: slug }],

    metadata: {
      source: "Paltamon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "paltamoKaavaResolver",
