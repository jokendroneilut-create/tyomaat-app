...
function mapKurikkaPhase(rawPhase: string | null): string {
...
export async function resolveKurikkaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKurikkaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kurikka_kaava_slug", value: slug }],

    metadata: {
      source: "Kurikan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kurikkaKaavaResolver",
