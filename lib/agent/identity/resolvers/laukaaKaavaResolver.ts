...
function mapLaukaaPhase(rawPhase: string | null): string {
...
export async function resolveLaukaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLaukaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "laukaa_kaava_slug", value: slug }],

    metadata: {
      source: "Laukaan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "laukaaKaavaResolver",
