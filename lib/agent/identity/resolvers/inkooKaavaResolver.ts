...
function mapInkooPhase(rawPhase: string | null): string {
...
export async function resolveInkooKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapInkooPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "inkoo_kaava_slug", value: slug }],

    metadata: {
      source: "Inkoon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "inkooKaavaResolver",
