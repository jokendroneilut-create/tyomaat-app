...
export async function resolveJyvaskylaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Jyväskylä")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Jyväskylä",
    ...
    identifiers: [{ type: "jyvaskyla_kaava_id", value: document.document_url }],

    metadata: {
      source: "Jyväskylän kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "jyvaskylaKaavaResolver",
