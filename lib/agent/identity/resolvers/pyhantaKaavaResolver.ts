...
function mapPyhantaPhase(rawPhase: string | null): string {
...
export async function resolvePyhantaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPyhantaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pyhanta_kaava_slug", value: slug }],

    metadata: {
      source: "Pyhännän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pyhantaKaavaResolver",
