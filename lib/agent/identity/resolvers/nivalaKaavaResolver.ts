...
function mapNivalaPhase(rawPhase: string | null): string {
...
export async function resolveNivalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapNivalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "nivala_kaava_slug", value: slug }],

    metadata: {
      source: "Nivalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "nivalaKaavaResolver",
