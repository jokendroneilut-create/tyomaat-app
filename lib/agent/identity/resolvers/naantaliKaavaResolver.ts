...
function mapNaantaliPhase(rawPhase: string | null): string {
  ...
  if (/luonno|valmistelu/.test(normalized)) return PHASE_LABELS.zoning
  return PHASE_LABELS.zoning
}

export async function resolveNaantaliKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapNaantaliPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "naantali_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Naantalin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "naantaliKaavaResolver",
