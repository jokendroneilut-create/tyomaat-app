...
function mapSonkajarviPhase(rawPhase: string | null): string {
...
export async function resolveSonkajarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSonkajarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sonkajarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Sonkajärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sonkajarviKaavaResolver",
