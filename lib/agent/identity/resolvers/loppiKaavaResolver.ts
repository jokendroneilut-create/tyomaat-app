...
function mapLoppiPhase(rawPhase: string | null): string {
...
export async function resolveLoppiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLoppiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "loppi_kaava_slug", value: slug }],

    metadata: {
      source: "Lopen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "loppiKaavaResolver",
