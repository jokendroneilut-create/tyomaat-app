...
function mapHuittinenPhase(rawPhase: string | null): string {
...
export async function resolveHuittinenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHuittinenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "huittinen_kaava_slug", value: slug }],

    metadata: {
      source: "Huittisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "huittinenKaavaResolver",
