...
function mapToivakkaPhase(rawPhase: string | null): string {
...
export async function resolveToivakkaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapToivakkaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "toivakka_kaava_slug", value: slug }],

    metadata: {
      source: "Toivakan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "toivakkaKaavaResolver",
