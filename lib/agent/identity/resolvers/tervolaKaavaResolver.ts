...
function mapTervolaPhase(rawPhase: string | null): string {
...
export async function resolveTervolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTervolaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "tervola_kaava_slug", value: slug }],

    metadata: {
      source: "Tervolan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "tervolaKaavaResolver",
