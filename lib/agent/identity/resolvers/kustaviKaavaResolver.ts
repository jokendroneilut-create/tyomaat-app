...
function mapKustaviPhase(rawPhase: string | null): string {
...
export async function resolveKustaviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKustaviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kustavi_kaava_slug", value: slug }],

    metadata: {
      source: "Kustavin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kustaviKaavaResolver",
