...
function mapKokemakiPhase(rawPhase: string | null): string {
...
export async function resolveKokemakiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKokemakiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kokemaki_kaava_slug", value: slug }],

    metadata: {
      source: "Kokemäen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kokemakiKaavaResolver",
