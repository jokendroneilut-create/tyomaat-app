...
function mapLemiPhase(rawPhase: string | null): string {
...
export async function resolveLemiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLemiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lemi_kaava_slug", value: slug }],

    metadata: {
      source: "Lemin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lemiKaavaResolver",
