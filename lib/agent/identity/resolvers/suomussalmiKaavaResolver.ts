...
function mapSuomussalmiPhase(rawPhase: string | null): string {
...
export async function resolveSuomussalmiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSuomussalmiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "suomussalmi_kaava_slug", value: slug }],

    metadata: {
      source: "Suomussalmen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "suomussalmiKaavaResolver",
