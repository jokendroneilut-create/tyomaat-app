...
function mapJamsaPhase(rawPhase: string | null): string {
...
export async function resolveJamsaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapJamsaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "jamsa_kaava_slug", value: slug }],

    metadata: {
      source: "Jämsän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "jamsaKaavaResolver",
