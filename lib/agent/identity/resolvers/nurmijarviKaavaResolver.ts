...
export async function resolveNurmijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Nurmijärvi")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Nurmijärvi",
    ...
    identifiers: [{ type: "nurmijarvi_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Nurmijärven kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "nurmijarviKaavaResolver",
