...
function mapYlivieskaPhase(rawPhase: string | null): string {
...
export async function resolveYlivieskaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapYlivieskaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ylivieska_kaava_slug", value: slug }],

    metadata: {
      source: "Ylivieskan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ylivieskaKaavaResolver",
