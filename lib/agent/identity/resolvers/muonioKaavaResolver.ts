...
function mapMuonioPhase(rawPhase: string | null): string {
...
export async function resolveMuonioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMuonioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "muonio_kaava_slug", value: slug }],

    metadata: {
      source: "Muonion kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "muonioKaavaResolver",
