...
function mapKarsamakiPhase(rawPhase: string | null): string {
...
export async function resolveKarsamakiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKarsamakiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "karsamaki_kaava_slug", value: slug }],

    metadata: {
      source: "Kärsämäen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "karsamakiKaavaResolver",
