...
function mapKittilaPhase(rawPhase: string | null): string {
...
export async function resolveKittilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKittilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kittila_kaava_slug", value: slug }],

    metadata: {
      source: "Kittilän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kittilaKaavaResolver",
