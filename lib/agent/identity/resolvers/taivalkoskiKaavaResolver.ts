...
function mapTaivalkoskiPhase(rawPhase: string | null): string {
...
export async function resolveTaivalkoskiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTaivalkoskiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "taivalkoski_kaava_slug", value: slug }],

    metadata: {
      source: "Taivalkosken kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "taivalkoskiKaavaResolver",
