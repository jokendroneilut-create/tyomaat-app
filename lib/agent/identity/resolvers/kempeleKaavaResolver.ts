...
function mapKempelePhase(rawPhase: string | null): string {
...
export async function resolveKempeleKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKempelePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kempele_kaava_slug", value: slug }],

    metadata: {
      source: "Kempeleen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kempeleKaavaResolver",
