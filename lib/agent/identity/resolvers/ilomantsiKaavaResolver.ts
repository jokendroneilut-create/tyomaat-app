...
function mapIlomantsiPhase(rawPhase: string | null): string {
...
export async function resolveIlomantsiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIlomantsiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ilomantsi_kaava_slug", value: slug }],

    metadata: {
      source: "Ilomantsin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ilomantsiKaavaResolver",
