...
function mapKinnulaPhase(rawPhase: string | null): string {
...
export async function resolveKinnulaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKinnulaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kinnula_kaava_slug", value: slug }],

    metadata: {
      source: "Kinnulan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kinnulaKaavaResolver",
