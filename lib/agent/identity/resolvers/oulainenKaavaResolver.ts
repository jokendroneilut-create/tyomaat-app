...
function mapOulainenPhase(rawPhase: string | null): string {
...
export async function resolveOulainenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapOulainenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "oulainen_kaava_slug", value: slug }],

    metadata: {
      source: "Oulaisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "oulainenKaavaResolver",
