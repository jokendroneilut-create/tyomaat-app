...
function mapVihtiPhase(rawPhase: string | null): string {
  ...
  if (/osallistumis|arviointi/.test(normalized)) return PHASE_LABELS.planning
  return PHASE_LABELS.zoning
}

export async function resolveVihtiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  /*
   * Vihdin sivusto listaa vain "vireillä olevat" kaavat — lainvoiman
   * saaneet kaavat poistuvat tästä listasta kokonaan, joten tähän
   * lähteeseen poimitut hankkeet eivät koskaan ole valmiita. Kaavan
   * vaihetekstissä voidaan mainita SISARKAAVAN (esim. V47a) saama
   * lainvoima, mikä tekisi tekstipohjaisesta "voimaantulo"-tunnistuksesta
   * epäluotettavan tälle sivustolle.
   */
  const completed = false
  const phaseHint = mapVihtiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "vihti_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Vihdin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vihtiKaavaResolver",
