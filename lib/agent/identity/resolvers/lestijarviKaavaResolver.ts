...
function mapLestijarviPhase(rawPhase: string | null): string {
...
export async function resolveLestijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLestijarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lestijarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Lestijärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lestijarviKaavaResolver",
