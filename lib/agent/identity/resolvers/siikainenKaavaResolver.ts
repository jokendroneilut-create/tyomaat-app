...
function mapSiikainenPhase(rawPhase: string | null): string {
...
export async function resolveSiikainenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSiikainenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "siikainen_kaava_slug", value: slug }],

    metadata: {
      source: "Siikaisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "siikainenKaavaResolver",
