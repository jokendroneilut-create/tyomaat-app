...
function mapKalajokiPhase(rawPhase: string | null): string {
...
export async function resolveKalajokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKalajokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kalajoki_kaava_slug", value: slug }],

    metadata: {
      source: "Kalajoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kalajokiKaavaResolver",
