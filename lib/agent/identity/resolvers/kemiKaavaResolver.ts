...
function mapKemiPhase(rawPhase: string | null): string {
...
export async function resolveKemiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKemiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kemi_kaava_slug", value: slug }],

    metadata: {
      source: "Kemin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kemiKaavaResolver",
