...
function mapKannusPhase(rawPhase: string | null): string {
...
export async function resolveKannusKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKannusPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kannus_kaava_slug", value: slug }],

    metadata: {
      source: "Kannuksen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kannusKaavaResolver",
