...
export async function resolveKokkolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Kokkola")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Kokkola",
    ...
    identifiers: [{ type: "kokkola_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Kokkolan kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kokkolaKaavaResolver",
