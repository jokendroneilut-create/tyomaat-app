...
function mapLapuaPhase(rawPhase: string | null): string {
...
export async function resolveLapuaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLapuaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lapua_kaava_slug", value: slug }],

    metadata: {
      source: "Lapuan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lapuaKaavaResolver",
