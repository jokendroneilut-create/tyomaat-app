...
function mapIsojokiPhase(rawPhase: string | null): string {
...
export async function resolveIsojokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIsojokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "isojoki_kaava_slug", value: slug }],

    metadata: {
      source: "Isojoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "isojokiKaavaResolver",
