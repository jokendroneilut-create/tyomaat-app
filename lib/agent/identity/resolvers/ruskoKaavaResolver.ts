...
function mapRuskoPhase(rawPhase: string | null): string {
...
export async function resolveRuskoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRuskoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "rusko_kaava_slug", value: slug }],

    metadata: {
      source: "Ruskon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ruskoKaavaResolver",
