...
function mapMerijarviPhase(rawPhase: string | null): string {
...
export async function resolveMerijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMerijarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "merijarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Merijärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "merijarviKaavaResolver",
