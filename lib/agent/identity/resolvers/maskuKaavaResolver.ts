...
function mapMaskuPhase(rawPhase: string | null): string {
...
export async function resolveMaskuKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMaskuPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "masku_kaava_slug", value: slug }],

    metadata: {
      source: "Maskun kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "maskuKaavaResolver",
