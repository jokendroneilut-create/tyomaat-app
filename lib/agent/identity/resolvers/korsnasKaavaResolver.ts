...
function mapKorsnasPhase(rawPhase: string | null): string {
...
export async function resolveKorsnasKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKorsnasPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "korsnas_kaava_slug", value: slug }],

    metadata: {
      source: "Korsnäsin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "korsnasKaavaResolver",
