...
function mapPelloPhase(rawPhase: string | null): string {
...
export async function resolvePelloKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPelloPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pello_kaava_slug", value: slug }],

    metadata: {
      source: "Pellon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pelloKaavaResolver",
