...
function mapTyrnavaPhase(rawPhase: string | null): string {
...
export async function resolveTyrnavaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTyrnavaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "tyrnava_kaava_slug", value: slug }],

    metadata: {
      source: "Tyrnävän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "tyrnavaKaavaResolver",
