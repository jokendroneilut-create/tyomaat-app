...
function mapKemionsaariPhase(rawPhase: string | null): string {
...
export async function resolveKemionsaariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKemionsaariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kemionsaari_kaava_slug", value: slug }],

    metadata: {
      source: "Kemiönsaaren kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kemionsaariKaavaResolver",
