...
function mapSastamalaPhase(rawPhase: string | null): string {
  ...
  if (/osallistumis|arviointi|oas/.test(normalized)) return PHASE_LABELS.zoning
  return PHASE_LABELS.zoning
}

export async function resolveSastamalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSastamalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sastamala_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Sastamalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sastamalaKaavaResolver",
