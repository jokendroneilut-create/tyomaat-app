...
function mapRuovesiPhase(rawPhase: string | null): string {
...
export async function resolveRuovesiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRuovesiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ruovesi_kaava_slug", value: slug }],

    metadata: {
      source: "Ruoveden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ruovesiKaavaResolver",
