...
function mapValkeakoskiPhase(rawPhase: string | null): string {
...
export async function resolveValkeakoskiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapValkeakoskiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "valkeakoski_kaava_slug", value: slug }],

    metadata: {
      source: "Valkeakosken kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "valkeakoskiKaavaResolver",
