...
function mapVieremaPhase(rawPhase: string | null): string {
...
export async function resolveVieremaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVieremaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "vierema_kaava_slug", value: slug }],

    metadata: {
      source: "Vieremän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vieremaKaavaResolver",
