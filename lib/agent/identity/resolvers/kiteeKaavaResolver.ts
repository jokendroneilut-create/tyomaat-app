...
function mapKiteePhase(rawPhase: string | null): string {
...
export async function resolveKiteeKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKiteePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kitee_kaava_slug", value: slug }],

    metadata: {
      source: "Kiteen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kiteeKaavaResolver",
