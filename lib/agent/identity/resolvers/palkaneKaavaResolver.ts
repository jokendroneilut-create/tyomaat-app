...
function mapPalkanePhase(rawPhase: string | null): string {
...
export async function resolvePalkaneKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPalkanePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "palkane_kaava_slug", value: slug }],

    metadata: {
      source: "Pälkäneen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "palkaneKaavaResolver",
