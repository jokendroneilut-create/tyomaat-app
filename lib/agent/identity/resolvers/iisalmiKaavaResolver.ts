...
function mapIisalmiPhase(rawPhase: string | null): string {
...
export async function resolveIisalmiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIisalmiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "iisalmi_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Iisalmen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "iisalmiKaavaResolver",
