...
function mapHausjarviPhase(rawPhase: string | null): string {
...
export async function resolveHausjarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHausjarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hausjarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Hausjärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hausjarviKaavaResolver",
