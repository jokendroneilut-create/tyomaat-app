...
function mapSallaPhase(rawPhase: string | null): string {
...
export async function resolveSallaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSallaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "salla_kaava_slug", value: slug }],

    metadata: {
      source: "Sallan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sallaKaavaResolver",
