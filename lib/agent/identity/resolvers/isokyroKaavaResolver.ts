...
function mapIsokyroPhase(rawPhase: string | null): string {
...
export async function resolveIsokyroKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapIsokyroPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "isokyro_kaava_slug", value: slug }],

    metadata: {
      source: "Isonkyrön kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "isokyroKaavaResolver",
