...
function mapHaapajarviPhase(rawPhase: string | null): string {
...
export async function resolveHaapajarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHaapajarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "haapajarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Haapajärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "haapajarviKaavaResolver",
