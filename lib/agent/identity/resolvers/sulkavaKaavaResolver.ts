...
function mapSulkavaPhase(rawPhase: string | null): string {
...
export async function resolveSulkavaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSulkavaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sulkava_kaava_slug", value: slug }],

    metadata: {
      source: "Sulkavan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sulkavaKaavaResolver",
