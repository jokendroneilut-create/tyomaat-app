...
function mapJanakkalaPhase(rawPhase: string | null): string {
...
export async function resolveJanakkalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapJanakkalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "janakkala_kaava_slug", value: slug }],

    metadata: {
      source: "Janakkalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "janakkalaKaavaResolver",
