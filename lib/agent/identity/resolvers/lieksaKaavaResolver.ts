...
function mapLieksaPhase(rawPhase: string | null): string {
...
export async function resolveLieksaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLieksaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lieksa_kaava_slug", value: slug }],

    metadata: {
      source: "Lieksan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lieksaKaavaResolver",
