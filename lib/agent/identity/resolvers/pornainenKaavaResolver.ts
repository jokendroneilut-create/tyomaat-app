...
function mapPornainenPhase(rawPhase: string | null): string {
...
export async function resolvePornainenKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPornainenPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pornainen_kaava_slug", value: slug }],

    metadata: {
      source: "Pornaisten kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pornainenKaavaResolver",
