...
function mapMuhosPhase(rawPhase: string | null): string {
...
export async function resolveMuhosKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMuhosPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "muhos_kaava_slug", value: slug }],

    metadata: {
      source: "Muhoksen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "muhosKaavaResolver",
