...
function mapLumijokiPhase(rawPhase: string | null): string {
...
export async function resolveLumijokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLumijokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "lumijoki_kaava_slug", value: slug }],

    metadata: {
      source: "Lumijoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "lumijokiKaavaResolver",
