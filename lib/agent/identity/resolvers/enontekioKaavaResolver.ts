...
function mapEnontekioPhase(rawPhase: string | null): string {
...
export async function resolveEnontekioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapEnontekioPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "enontekio_kaava_slug", value: slug }],

    metadata: {
      source: "Enontekiön kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "enontekioKaavaResolver",
