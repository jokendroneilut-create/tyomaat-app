...
function mapPyhajokiPhase(rawPhase: string | null): string {
...
export async function resolvePyhajokiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPyhajokiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pyhajoki_kaava_slug", value: slug }],

    metadata: {
      source: "Pyhäjoen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pyhajokiKaavaResolver",
