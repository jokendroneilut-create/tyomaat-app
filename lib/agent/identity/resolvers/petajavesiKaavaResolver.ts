...
function mapPetajavesiPhase(rawPhase: string | null): string {
...
export async function resolvePetajavesiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPetajavesiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "petajavesi_kaava_slug", value: slug }],

    metadata: {
      source: "Petäjäveden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "petajavesiKaavaResolver",
