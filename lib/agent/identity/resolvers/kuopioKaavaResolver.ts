...
import { gk27ToWgs84 } from "@/lib/geo/gk27"

function findFact(facts: any[], type: string) {
  return facts.find((fact) => fact.fact_type === type)
}

export async function resolveKuopioKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Kuopio",
    address: null,
    propertyId: null,
    permitNumber: null,
    sourceName: document.source_name,

    identifiers: [{ type: "kuopio_kaava_tunnus", value: kaavaTunnus ?? recordNumber }],

    metadata: {
      source: "Kuopion kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kuopioKaavaResolver",
