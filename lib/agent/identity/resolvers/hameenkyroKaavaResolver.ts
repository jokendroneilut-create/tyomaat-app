...
function mapHameenkyroPhase(rawPhase: string | null): string {
...
export async function resolveHameenkyroKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHameenkyroPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hameenkyro_kaava_slug", value: slug }],

    metadata: {
      source: "Hämeenkyrön kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hameenkyroKaavaResolver",
