...
function mapRistijarviPhase(rawPhase: string | null): string {
...
export async function resolveRistijarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapRistijarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "ristijarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Ristijärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "ristijarviKaavaResolver",
