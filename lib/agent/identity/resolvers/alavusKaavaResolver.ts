...
function mapAlavusPhase(rawPhase: string | null): string {
...
export async function resolveAlavusKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAlavusPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "alavus_kaava_slug", value: slug }],

    metadata: {
      source: "Alavuden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "alavusKaavaResolver",
