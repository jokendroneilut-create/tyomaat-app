...
export async function resolveKeravaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const municipality = getMunicipalityByName("Kerava")
  ...
  const result = await resolvePotentialProject({
    title: operation,
    municipality: municipality?.name ?? "Kerava",
    ...
    identifiers: [{ type: "kerava_kaava_tunnus", value: kaavaTunnus }],

    metadata: {
      source: "Keravan kaupunkisuunnittelu",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "keravaKaavaResolver",
