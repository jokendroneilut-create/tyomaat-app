...
function mapPyhajarviPhase(rawPhase: string | null): string {
...
export async function resolvePyhajarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapPyhajarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "pyhajarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Pyhäjärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "pyhajarviKaavaResolver",
