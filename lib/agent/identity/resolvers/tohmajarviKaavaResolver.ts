...
function mapTohmajarviPhase(rawPhase: string | null): string {
...
export async function resolveTohmajarviKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapTohmajarviPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "tohmajarvi_kaava_slug", value: slug }],

    metadata: {
      source: "Tohmajärven kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "tohmajarviKaavaResolver",
