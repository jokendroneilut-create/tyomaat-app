...
function mapLoviisaPhase(rawPhase: string | null): string {
...
export async function resolveLoviisaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapLoviisaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "loviisa_kaava_slug", value: slug }],

    metadata: {
      source: "Loviisan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "loviisaKaavaResolver",
