...
function mapSodankylaPhase(rawPhase: string | null): string {
...
export async function resolveSodankylaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSodankylaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "sodankyla_kaava_slug", value: slug }],

    metadata: {
      source: "Sodankylän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "sodankylaKaavaResolver",
