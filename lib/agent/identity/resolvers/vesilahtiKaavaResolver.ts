...
function mapVesilahtiPhase(rawPhase: string | null): string {
...
export async function resolveVesilahtiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVesilahtiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "vesilahti_kaava_slug", value: slug }],

    metadata: {
      source: "Vesilahden kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vesilahtiKaavaResolver",
