...
function mapKarkkilaPhase(rawPhase: string | null): string {
...
export async function resolveKarkkilaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKarkkilaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "karkkila_kaava_slug", value: slug }],

    metadata: {
      source: "Karkkilan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "karkkilaKaavaResolver",
