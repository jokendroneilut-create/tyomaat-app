...
function mapAsikkalaPhase(rawPhase: string | null): string {
...
export async function resolveAsikkalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAsikkalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "asikkala_kaava_slug", value: slug }],

    metadata: {
      source: "Asikkalan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "asikkalaKaavaResolver",
