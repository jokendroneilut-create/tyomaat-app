...
function mapViitasaariPhase(rawPhase: string | null): string {
...
export async function resolveViitasaariKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapViitasaariPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "viitasaari_kaava_slug", value: slug }],

    metadata: {
      source: "Viitasaaren kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "viitasaariKaavaResolver",
