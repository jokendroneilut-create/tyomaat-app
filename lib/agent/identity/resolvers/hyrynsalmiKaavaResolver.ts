...
function mapHyrynsalmiPhase(rawPhase: string | null): string {
...
export async function resolveHyrynsalmiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHyrynsalmiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hyrynsalmi_kaava_slug", value: slug }],

    metadata: {
      source: "Hyrynsalmen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hyrynsalmiKaavaResolver",
