...
function mapHartolaPhase(rawPhase: string | null): string {
...
export async function resolveHartolaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapHartolaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "hartola_kaava_slug", value: slug }],

    metadata: {
      source: "Hartolan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "hartolaKaavaResolver",
