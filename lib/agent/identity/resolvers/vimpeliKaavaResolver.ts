...
function mapVimpeliPhase(rawPhase: string | null): string {
...
export async function resolveVimpeliKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVimpeliPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "vimpeli_kaava_slug", value: slug }],

    metadata: {
      source: "Vimpelin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "vimpeliKaavaResolver",
