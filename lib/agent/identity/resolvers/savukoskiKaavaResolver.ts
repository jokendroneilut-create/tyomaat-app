...
function mapSavukoskiPhase(rawPhase: string | null): string {
...
export async function resolveSavukoskiKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSavukoskiPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "savukoski_kaava_slug", value: slug }],

    metadata: {
      source: "Savukosken kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "savukoskiKaavaResolver",
