...
function mapVoyriPhase(rawPhase: string | null): string {
...
export async function resolveVoyriKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapVoyriPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "voyri_kaava_slug", value: slug }],

    metadata: {
      source: "Vöyrin kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "voyriKaavaResolver",
