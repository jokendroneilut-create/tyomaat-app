...
function mapMiehikkalaPhase(rawPhase: string | null): string {
...
export async function resolveMiehikkalaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapMiehikkalaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "miehikkala_kaava_slug", value: slug }],

    metadata: {
      source: "Miehikkälän kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "miehikkalaKaavaResolver",
