...
function mapKuortanePhase(rawPhase: string | null): string {
...
export async function resolveKuortaneKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKuortanePhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "kuortane_kaava_slug", value: slug }],

    metadata: {
      source: "Kuortaneen kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "kuortaneKaavaResolver",
