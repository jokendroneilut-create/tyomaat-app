...
function mapSimoPhase(rawPhase: string | null): string {
...
export async function resolveSimoKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapSimoPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "simo_kaava_slug", value: slug }],

    metadata: {
      source: "Simon kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "simoKaavaResolver",
