...
function mapKeuruuPhase(rawPhase: string | null): string {
...
export async function resolveKeuruuKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapKeuruuPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "keuruu_kaava_slug", value: slug }],

    metadata: {
      source: "Keuruun kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "keuruuKaavaResolver",
