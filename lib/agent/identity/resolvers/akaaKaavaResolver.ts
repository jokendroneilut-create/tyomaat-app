...
function mapAkaaPhase(rawPhase: string | null): string {
...
export async function resolveAkaaKaavaProject({
  document,
  facts,
}: {
  document: any
  ...
  const phaseHint = mapAkaaPhase(phase)
  ...
  const result = await resolvePotentialProject({
    ...
    identifiers: [{ type: "akaa_kaava_slug", value: slug }],

    metadata: {
      source: "Akaan kaavoitus",
      source_name: document.source_name,
      source_document_id: document.id,
      resolver: "akaaKaavaResolver",
