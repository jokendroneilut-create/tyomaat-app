...
export function extractPieksamakiKaavaFacts({
  documentId,
  sourceName,
  title,
  phase,
  ...
  const operation = clean(title) ?? "Pieksämäen kaava"

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "pieksamakiKaavaParser",

    decision_index: title ?? "pieksamaki-kaava",
