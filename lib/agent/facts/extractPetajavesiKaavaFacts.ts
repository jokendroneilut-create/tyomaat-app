...
export function extractPetajavesiKaavaFacts({
  documentId,
  sourceName,
  title,
  phase,
  ...
  const operation = clean(title) ?? "Petäjäveden kaava"

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "petajavesiKaavaParser",

    decision_index: title ?? "petajavesi-kaava",
