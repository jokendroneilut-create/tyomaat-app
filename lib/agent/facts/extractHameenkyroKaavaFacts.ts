...
export function extractHameenkyroKaavaFacts({
  documentId,
  sourceName,
  title,
  phase,
  ...
  const operation = clean(title) ?? "Hämeenkyrön kaava"

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "hameenkyroKaavaParser",

    decision_index: title ?? "hameenkyro-kaava",
