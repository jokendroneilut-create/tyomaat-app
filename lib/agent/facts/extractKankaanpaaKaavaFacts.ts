...
export function extractKankaanpaaKaavaFacts({
  documentId,
  sourceName,
  title,
  kaavaTunnus,
  ...
  const operation = clean(title) ?? `Kankaanpään kaava ${kaavaTunnus ?? "?"}`

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "kankaanpaaKaavaParser",

    decision_index: kaavaTunnus ?? title ?? "kankaanpaa-kaava",
