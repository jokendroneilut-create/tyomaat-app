...
export function extractNokiaKaavaFacts({
  documentId,
  sourceName,
  title,
  kaavaTunnus,
  ...
  const operation = clean(address) ?? clean(title) ?? `Kaava ${kaavaTunnus ?? "?"}`

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "nokiaKaavaParser",

    decision_index: kaavaTunnus ?? title ?? "nokia-kaava",
