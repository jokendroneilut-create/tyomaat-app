...
export function extractSaarijarviKaavaFacts({
  documentId,
  sourceName,
  title,
  phase,
  ...
  const operation = clean(title) ?? "Saarijärven kaava"

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "saarijarviKaavaParser",

    decision_index: title ?? "saarijarvi-kaava",
