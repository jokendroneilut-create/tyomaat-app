...
export function extractLapuaKaavaFacts({
  documentId,
  sourceName,
  title,
  kaavaTunnus,
  ...
  const operation = clean(title) ?? `Lapuan kaava ${kaavaTunnus ?? "?"}`

  const commonMetadata = {
    source_document_id: documentId,
    source_name: sourceName,
    parser: "lapuaKaavaParser",

    decision_index: kaavaTunnus ?? title ?? "lapua-kaava",
