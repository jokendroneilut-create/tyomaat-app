...
import { extractKeminmaaKaavaFacts } from "@/lib/agent/facts/extractKeminmaaKaavaFacts"
import { extractMuonioKaavaFacts } from "@/lib/agent/facts/extractMuonioKaavaFacts"
import { extractPelkosenniemiKaavaFacts } from "@/lib/agent/facts/extractPelkosenniemiKaavaFacts"
...
import { extractPalkaneKaavaFacts } from "@/lib/agent/facts/extractPalkaneKaavaFacts"
import { extractVesilahtiKaavaFacts } from "@/lib/agent/facts/extractVesilahtiKaavaFacts"
import { extractKaskinenKaavaFacts } from "@/lib/agent/facts/extractKaskinenKaavaFacts"
import { extractRuovesiKaavaFacts } from "@/lib/agent/facts/extractRuovesiKaavaFacts"
import { extractVirratKaavaFacts } from "@/lib/agent/facts/extractVirratKaavaFacts"
...
import { extractSiikainenKaavaFacts } from "@/lib/agent/facts/extractSiikainenKaavaFacts"
import { extractJoutsaKaavaFacts } from "@/lib/agent/facts/extractJoutsaKaavaFacts"
import { extractPielavesiKaavaFacts } from "@/lib/agent/facts/extractPielavesiKaavaFacts"
import { extractKiuruvesiKaavaFacts } from "@/lib/agent/facts/extractKiuruvesiKaavaFacts"
import { extractAuraKaavaFacts } from "@/lib/agent/facts/extractAuraKaavaFacts"
import { extractVehmaaKaavaFacts } from "@/lib/agent/facts/extractVehmaaKaavaFacts"
import { extractLaitilaKaavaFacts } from "@/lib/agent/facts/extractLaitilaKaavaFacts"
import { extractKustaviKaavaFacts } from "@/lib/agent/facts/extractKustaviKaavaFacts"
...
import { extractOulainenKaavaFacts } from "@/lib/agent/facts/extractOulainenKaavaFacts"
import { extractTaivalkoskiKaavaFacts } from "@/lib/agent/facts/extractTaivalkoskiKaavaFacts"
import { extractPoytyaKaavaFacts } from "@/lib/agent/facts/extractPoytyaKaavaFacts"
import { extractMaskuKaavaFacts } from "@/lib/agent/facts/extractMaskuKaavaFacts"
import { extractRuskoKaavaFacts } from "@/lib/agent/facts/extractRuskoKaavaFacts"
import { extractMynamakiKaavaFacts } from "@/lib/agent/facts/extractMynamakiKaavaFacts"
import { extractKemionsaariKaavaFacts } from "@/lib/agent/facts/extractKemionsaariKaavaFacts"
import { extractMarttilaKaavaFacts } from "@/lib/agent/facts/extractMarttilaKaavaFacts"
import { extractPyharantaKaavaFacts } from "@/lib/agent/facts/extractPyharantaKaavaFacts"
import { extractTaivassaloKaavaFacts } from "@/lib/agent/facts/extractTaivassaloKaavaFacts"
import { extractKristiinankaupunkiKaavaFacts } from "@/lib/agent/facts/extractKristiinankaupunkiKaavaFacts"
...
import { extractPerhoKaavaFacts } from "@/lib/agent/facts/extractPerhoKaavaFacts"
import { extractLestijarviKaavaFacts } from "@/lib/agent/facts/extractLestijarviKaavaFacts"
import { extractIlomantsiKaavaFacts } from "@/lib/agent/facts/extractIlomantsiKaavaFacts"
import { extractTohmajarviKaavaFacts } from "@/lib/agent/facts/extractTohmajarviKaavaFacts"
...
import { extractTaipalsaariKaavaFacts } from "@/lib/agent/facts/extractTaipalsaariKaavaFacts"
import { extractKinnulaKaavaFacts } from "@/lib/agent/facts/extractKinnulaKaavaFacts"
...
import { extractKarsamakiKaavaFacts } from "@/lib/agent/facts/extractKarsamakiKaavaFacts"
import { extractLumijokiKaavaFacts } from "@/lib/agent/facts/extractLumijokiKaavaFacts"
...
import { extractPudasjarviKaavaFacts } from "@/lib/agent/facts/extractPudasjarviKaavaFacts"
import { extractReisjarviKaavaFacts } from "@/lib/agent/facts/extractReisjarviKaavaFacts"
...
import { extractSysmaKaavaFacts } from "@/lib/agent/facts/extractSysmaKaavaFacts"
import { extractKoskiTlKaavaFacts } from "@/lib/agent/facts/extractKoskiTlKaavaFacts"
...
export function resolveFacts(document: any) {
  ...
  if (document.source_name === "Hangon ajankohtaiset kaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Inkoon ajankohtainen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Siikaisten kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractSiikainenKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Joutsan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Pielaveden kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractPielavesiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kiuruveden kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKiuruvesiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Auran kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Vehmaan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Laitilan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractLaitilaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kustavin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKustaviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Sievin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Vaalan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Siikajoen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Iin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Alavieskan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Oulaisten kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractOulainenKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Taivalkosken kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractTaivalkoskiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Pöytyän kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Maskun kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Ruskon kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Mynämäen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractMynamakiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kemiönsaaren kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKemionsaariKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Marttilan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractMarttilaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Pyhärannan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractPyharantaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Taivassalon kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractTaivassaloKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kristiinankaupungin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKristiinankaupunkiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Korsnäsin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKorsnasKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kruunupyyn kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKruunupyyKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Luodon kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Pedersören kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractPedersoreKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Uusikaarlepyyn kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Perhon kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Lestijärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractLestijarviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Ilomantsin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractIlomantsiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Tohmajärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractTohmajarviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Tammelan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractTammelaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Lemin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Kangasniemen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Kolarin kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Sallan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Jämijärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractJamijarviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Nakkilan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractNakkilaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Säkylän kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Taipalsaaren kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractTaipalsaariKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kinnulan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKinnulaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Nurmeksen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Merijärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractMerijarviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Haapajärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Kärsämäen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKarsamakiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Lumijoen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractLumijokiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Muhoksen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Pyhäjoen kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractPyhajokiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Pyhäjärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Pudasjärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractPudasjarviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Reisjärven kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractReisjarviKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Tyrnävän kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractTyrnavaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Asikkalan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractAsikkalaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Hartolan kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Sysmän kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
  ...
  if (document.source_name === "Kosken Tl kaavoitus") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    ...
    return {
      decisions: [],
      facts: extractKoskiTlKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Rajukivi Oy") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const municipality = document.raw_payload?.municipality ?? null

    return {
      decisions: [],
      facts: extractRajukiviFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Keminmaan vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractKeminmaaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Muonion vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
  ...
  if (document.source_name === "Pelkosenniemen vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractPelkosenniemiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Ranuan vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
  ...
  if (document.source_name === "Simon vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
  ...
  if (document.source_name === "Ylitornion vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractYlitornioKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Hämeenkyrön vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
  ...
  if (document.source_name === "Mänttä-Vilppulan vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
  ...
  if (document.source_name === "Pälkäneen vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractPalkaneKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Vesilahden vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractVesilahtiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Kaskisten vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractKaskinenKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Ruoveden vireillä olevat kaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractRuovesiKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
  ...
  if (document.source_name === "Virtain vireillä olevat kaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
  ...
  if (document.source_name === "Sodankylän vireillä olevat asemakaavat") {
    const title = document.raw_payload?.title ?? document.title ?? null
    const phase = document.raw_payload?.phase ?? null
    const description = document.raw_payload?.description ?? null
    const contacts = document.raw_payload?.contacts ?? []
    ...
    return {
      decisions: [],
      facts: extractSodankylaKaavaFacts({
        documentId: document.id,
        sourceName: document.source_name,
        title,
        phase,
