...
import { resolveTaipalsaariKaavaProject } from "@/lib/agent/identity/resolvers/taipalsaariKaavaResolver"
import { resolveKinnulaKaavaProject } from "@/lib/agent/identity/resolvers/kinnulaKaavaResolver"
...
import { resolveKarsamakiKaavaProject } from "@/lib/agent/identity/resolvers/karsamakiKaavaResolver"
import { resolveLumijokiKaavaProject } from "@/lib/agent/identity/resolvers/lumijokiKaavaResolver"
import { resolveMuhosKaavaProject } from "@/lib/agent/identity/resolvers/muhosKaavaResolver"
import { resolvePyhajokiKaavaProject } from "@/lib/agent/identity/resolvers/pyhajokiKaavaResolver"
import { resolvePyhajarviKaavaProject } from "@/lib/agent/identity/resolvers/pyhajarviKaavaResolver"
import { resolvePyhantaKaavaProject } from "@/lib/agent/identity/resolvers/pyhantaKaavaResolver"
import { resolvePudasjarviKaavaProject } from "@/lib/agent/identity/resolvers/pudasjarviKaavaResolver"
import { resolveReisjarviKaavaProject } from "@/lib/agent/identity/resolvers/reisjarviKaavaResolver"
import { resolveTyrnavaKaavaProject } from "@/lib/agent/identity/resolvers/tyrnavaKaavaResolver"
...
import { resolveSysmaKaavaProject } from "@/lib/agent/identity/resolvers/sysmaKaavaResolver"
import { resolveKoskiTlKaavaProject } from "@/lib/agent/identity/resolvers/koskiTlKaavaResolver"
import { resolveRajukiviProject } from "@/lib/agent/identity/resolvers/rajukiviResolver"
...
if (sourceName === "hilma") {
    ...
    results.push(result)
  } else if (sourceName === "taipalsaaren kaavoitus") {
    const result = await resolveTaipalsaariKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "kinnulan kaavoitus") {
    const result = await resolveKinnulaKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "nurmeksen kaavoitus") {
    const result = await resolveNurmesKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "merijärven kaavoitus") {
    const result = await resolveMerijarviKaavaProject({
      document,
      facts: facts ?? [],
    })
    ...
    results.push(result)
  } else if (sourceName === "kärsämäen kaavoitus") {
    const result = await resolveKarsamakiKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "lumijoen kaavoitus") {
    const result = await resolveLumijokiKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "muhoksen kaavoitus") {
    const result = await resolveMuhosKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "pyhäjoen kaavoitus") {
    const result = await resolvePyhajokiKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "pyhäjärven kaavoitus") {
    const result = await resolvePyhajarviKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "pyhännän kaavoitus") {
    const result = await resolvePyhantaKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "pudasjärven kaavoitus") {
    const result = await resolvePudasjarviKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "reisjärven kaavoitus") {
    const result = await resolveReisjarviKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "tyrnävän kaavoitus") {
    const result = await resolveTyrnavaKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "asikkalan kaavoitus") {
    const result = await resolveAsikkalaKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "hartolan kaavoitus") {
    const result = await resolveHartolaKaavaProject({
      document,
      facts: facts ?? [],
    })
    ...
    results.push(result)
  } else if (sourceName === "sysmän kaavoitus") {
    const result = await resolveSysmaKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "kosken tl kaavoitus") {
    const result = await resolveKoskiTlKaavaProject({
      document,
      facts: facts ?? [],
    })

    results.push(result)
  } else if (sourceName === "rajukivi oy") {
    const result = await resolveRajukiviProject({
      document,
      facts: facts ?? [],
    })
