...
const KANNUS_LISTING_URL = "https://kannus.fi/asuminen-ja-ymparisto/kaavoitus-ja-kartat/kaavahankkeet/"

// name/title/phone/email read directly off the page's own contact card
// (verified, not guessed)
const KANNUS_CONTACT = {
  name: "Helmi Pölkki",
  title: "Yhdyskuntapäällikkö",
  phone: "+358 44 474 5247",
  email: "helmi.polkki@kannus.fi",
}

function kannusPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectKannusKaavaSource(source: DiscoverySource) {
  const response = await fetch(KANNUS_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  // the site injects soft-hyphen characters (U+00AD) throughout its own
  // text for line-wrapping hints, so every piece of text pulled from
  // this page has them stripped before use
  const stripSoftHyphens = (text: string) => text.replace(/­/g, "")

  // every plan is its own flat <p>: bolded title, then one <a> per
  // attachment separated by <br> -- stripping the <a> tags out of a
  // clone leaves just the title text (works even for the two paragraphs
  // where the site's own markup slipped the leading letter outside the
  // <strong> tag)
  const h2 = $("h2")
    .toArray()
    .find((el) => stripSoftHyphens($(el).text()).trim() === "Vireillä olevat kaavahankkeet")
  if (!h2) return { documentsFound: 0, documentsSaved: 0 }

  let found = 0
  let saved = 0

  for (const pEl of $(h2).nextUntil("h2").filter("p").toArray()) {
    const original = $(pEl)
    const attachments = original
      .find("a")
      .toArray()
      .map((a) => ({
        label: stripSoftHyphens($(a).text()).replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", KANNUS_LISTING_URL).toString(),
      }))

    // the title is always the first line, but a couple of paragraphs
    // have a stray letter sitting outside the <a> tag partway through
    // (a site markup slip -- e.g. "...<br>K<a>aavaselostus...") that
    // would otherwise leak into a naive "strip all <a> tags" extraction,
    // so only text nodes/tags before the first <br> are taken
    const titleParts: string[] = []
    for (const node of original.contents().toArray()) {
      if (node.type === "tag" && node.name === "br") break
      titleParts.push($(node).text())
    }
    ...
    const completed = phase === "Voimaantulo"
    const contacts = [KANNUS_CONTACT]

    found += 1

    const slug = kemiSlug(title)
    const documentUrl = `${KANNUS_LISTING_URL}#${slug}`
...
const SUOMUSSALMI_LISTING_URL = "https://suomussalmi.fi/fi/asuminen-ja-ymparisto/kaavoitus/vireilla-olevat-kaavat"

// both read directly off the page's own per-plan contact footer
// (verified, not guessed)
const SUOMUSSALMI_CONTACTS = [
  { name: "Niina Kinnunen", title: "Tekninen johtaja", phone: "044 777 3380", email: "niina.j.kinnunen@suomussalmi.fi" },
  { name: "Jarkko Juntunen", title: "Maanmittausinsinööri", phone: "044 568 4335", email: "jarkko.juntunen@suomussalmi.fi" },
]

function suomussalmiPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectSuomussalmiKaavaSource(source: DiscoverySource) {
  const response = await fetch(SUOMUSSALMI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const items = $("details.accordion-details-item").toArray()
  ...
  for (const itemEl of items) {
    ...
    const phase = suomussalmiPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = SUOMUSSALMI_CONTACTS

    const attachments = bodyEl
      .find("a")
      .toArray()
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", SUOMUSSALMI_LISTING_URL).toString(),
      }))

    found += 1

    const slug = kemiSlug(title)
    const documentUrl = `${SUOMUSSALMI_LISTING_URL}#${slug}`
...
const KITTILA_LISTING_URL = "https://kittila.fi/asuminen-ja-ymparisto/kaavoitus/vireilla-olevat-kaavat"

// the site explicitly labels this firm/person as "Kaavanlaatija ja
// yhteyshenkilö kaavaan liittyvissä kysymyksissä" (plan preparer and
// designated contact for questions) on several of the plan entries
// themselves -- verified, not guessed
const KITTILA_CONTACT = {
  name: "Jarmo Lokio",
  title: "Arkkitehtitoimisto Jarmo Lokio Oy, kaavanlaatija",
  phone: "044 700 2155",
  email: "jarmo.lokio@lokio.fi",
}

function kittilaPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectKittilaKaavaSource(source: DiscoverySource) {
  const response = await fetch(KITTILA_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }
  ...
  for (const el of $("h3").first().parent().children().toArray()) {
    if (el.name === "h3") {
      const title = $(el).text().replace(/\s+/g, " ").trim()
  ...
  for (const block of blocks) {
    ...
    const phase = kittilaPhaseFromText(`${block.title} ${bodyText}`)
    const completed = phase === "Voimaantulo"
    const contacts = [KITTILA_CONTACT]

    const attachments = block.nodes
      .flatMap((node) => $(node).find("a").toArray())
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", KITTILA_LISTING_URL).toString(),
      }))
...
const RAUTJARVI_LISTING_URL =
  "https://www.rautjarvi.fi/fi/asuminen-ja-ymparisto/kaavoitus-ja-maankaytto/vireilla-olevat-kaavat"

// name/title/phone/email read directly off a published Dynasty
// pöytäkirja (verified, not guessed)
const RAUTJARVI_CONTACT = {
  name: "Juho Jylhä",
  title: "Tekninen johtaja",
  phone: "0400 654 897",
  email: "juho.jylha@rautjarvi.fi",
}

function rautjarviPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectRautjarviKaavaSource(source: DiscoverySource) {
  const response = await fetch(RAUTJARVI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }
  ...
  for (const itemEl of items) {
    ...
    const phase = rautjarviPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [RAUTJARVI_CONTACT]
    ...
    const attachments = bodyEl
      ...
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", RAUTJARVI_LISTING_URL).toString(),
      }))

    found += 1

    const slug = kemiSlug(title)
    const documentUrl = `${RAUTJARVI_LISTING_URL}#${slug}`
...
const ALAVUS_HUB_URL =
  "https://www.alavus.fi/fi/asuminen-ja-ymparisto/kaavoitus/ajankohtaista-kaavoituksessa.html"

const ALAVUS_CONTACT = {
  name: "Alavuden kaupunki, kaavoitus",
  title: "Kirjaamo",
  phone: "(06) 2525 1000",
  email: "info@alavus.fi",
}

function alavusSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function alavusPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
  if (!negatedLainvoima && /voimaantulo|lainvoima|tulee voimaan/.test(normalized)) return "Voimaantulo"
...
// Alavus has no single "vireillä olevat kaavat" listing page — instead a
// hub page ("Ajankohtaista kaavoituksessa") whose site-nav sidebar lists
// each individual plan as its own static detail page. We read the hub's
// own nav container (found via its self-link) for the list of sub-pages,
// then fetch each matching detail page directly.
async function collectAlavusKaavaSource(source: DiscoverySource) {
  const response = await fetch(ALAVUS_HUB_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const selfLink = $('a[href$="ajankohtaista-kaavoituksessa.html"]').first()
  const container = selfLink.parent().parent()
  ...
  const items = container
    ...
    .filter((el) => {
      ...
      return href.includes("/ajankohtaista-kaavoituksessa/") && /asemakaava/i.test(title)
    })
  ...
  for (const el of items) {
    ...
    const description = mainContent.text().replace(/\s+/g, " ").trim()

    const phase = alavusPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [ALAVUS_CONTACT]

    const attachments = mainContent
      .find("a")
      .toArray()
      .map((a) => ({
        label: $$(a).text().replace(/\s+/g, " ").trim(),
...
const KUORTANE_LISTING_URL =
  "https://kuortane.fi/rakentaminen-ja-ymparisto/rakennusvalvonta/vireilla-olevat-kaavat/"

// name/title/phone/email read directly off the published rakennusvalvonta
// contact info (verified, not guessed)
const KUORTANE_CONTACT = {
  name: "Katja Karonen",
  title: "Rakennustarkastaja",
  phone: "0400 179 467",
  email: "rakennustarkastaja@kuortane.fi",
}

function kuortaneSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function kuortanePhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectKuortaneKaavaSource(source: DiscoverySource) {
  const response = await fetch(KUORTANE_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const headings = $(".page-content").find("h2").toArray()
  ...
  for (const headingEl of headings) {
    ...
    const phase = kuortanePhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [KUORTANE_CONTACT]
    ...
    const attachments = blockNodes
      ...
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", KUORTANE_LISTING_URL).toString(),
      }))

    found += 1

    const slug = kuortaneSlug(title)
    const documentUrl = `${KUORTANE_LISTING_URL}#${slug}`
...
const ENONKOSKI_LISTING_URL = "https://enonkoski.fi/asukkaille/asuminen-ja-ymparisto/kaavoitus/"

const ENONKOSKI_CONTACT = {
  name: "Enonkosken kunta, tekninen toimi",
  title: "Tekninen toimi",
  phone: "044 345 3026",
  email: "tekninen.toimisto@enonkoski.fi",
}

function enonkoskiSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function enonkoskiPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
// The "Nähtävillä olevat kaavat" heading is followed by true document-flow
// sibling <p>/<ul> nodes (until the next h2). Title-marker paragraphs have
// no direct <a> child; content paragraphs and <ul> lists carry attachment
// links for the current block.
async function collectEnonkoskiKaavaSource(source: DiscoverySource) {
  const response = await fetch(ENONKOSKI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const heading = $("h2")
    .filter((_, el) => $(el).text().trim() === "Nähtävillä olevat kaavat")
    .first()
  if (heading.length === 0) return { documentsFound: 0, documentsSaved: 0 }
      ...
      if (el.tagName === "p") {
        ...
        if (directLink.length > 0) {
          if (current) {
            current.links.push({
              label: directLink.text().replace(/\s+/g, " ").trim(),
              url: new URL(directLink.attr("href") ?? "", ENONKOSKI_LISTING_URL).toString(),
            })
          }
          return
        }
        ...
        if (current) blocks.push(current)
        current = { title: text.replace(/:$/, ""), links: [] }
      } else if (el.tagName === "ul" && current) {
        $(el)
          .find("a")
          .each((_, a) => {
            current!.links.push({
              label: $(a).text().replace(/\s+/g, " ").trim(),
              url: new URL($(a).attr("href") ?? "", ENONKOSKI_LISTING_URL).toString(),
            })
          })
      }
    })
  ...
  for (const block of blocks) {
    const title = block.title
    if (!title || /yleiskaava/i.test(title) || /ranta-asemakaava/i.test(title)) continue

    const description = block.links.map((l) => l.label).join(" ")
    const phase = enonkoskiPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [ENONKOSKI_CONTACT]
    const attachments = block.links

    found += 1

    const slug = enonkoskiSlug(title)
    const documentUrl = `${ENONKOSKI_LISTING_URL}#${slug}`
...
const HEINAVESI_LISTING_URL = "https://www.heinavesi.fi/asuminen-ja-ymparisto/kaavoitus-ja-maankaytto/"

const HEINAVESI_CONTACT = {
  name: "Mikko Luostarinen",
  title: "Tekninen johtaja",
  phone: "040 355 6320",
  email: "mikko.luostarinen@heinavesi.fi",
}

function heinavesiSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function heinavesiPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectHeinavesiKaavaSource(source: DiscoverySource) {
  const response = await fetch(HEINAVESI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }
  ...
  for (const headingEl of headings) {
    const heading = $(headingEl)
    // The site injects soft hyphens (U+00AD) mid-word for hyphenation,
    // which silently breaks substring matches like /asemakaava/ (e.g.
    // "asema­kaavan") unless stripped first.
    const title = heading.text().replace(/­/g, "").replace(/\s+/g, " ").trim()
    if (
      !title ||
      !/asemakaava/i.test(title) ||
      /yleiskaava/i.test(title) ||
      /ranta-asemakaava/i.test(title) ||
      /voit ladata/i.test(title) // generic "download the map here" label, not a plan
    ) {
      continue
    }
    ...
    const phase = heinavesiPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [HEINAVESI_CONTACT]
    ...
    const attachments = blockNodes
      ...
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", HEINAVESI_LISTING_URL).toString(),
      }))

    found += 1

    const slug = heinavesiSlug(title)
    const documentUrl = `${HEINAVESI_LISTING_URL}#${slug}`
...
const SULKAVA_LISTING_URL = "https://sulkava.fi/asuminen-ja-ymparisto/kaavoitus/vireilla-olevat-kaavat/"

const SULKAVA_CONTACT = {
  name: "Asta Veikkanen",
  title: "Tekninen johtaja",
  phone: "044 417 5230",
  email: "asta.veikkanen@sulkava.fi",
}

function sulkavaSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function sulkavaPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectSulkavaKaavaSource(source: DiscoverySource) {
  const response = await fetch(SULKAVA_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const headings = $(".entry-content").find("h2").toArray()
  ...
  for (const headingEl of headings) {
    ...
    const phase = sulkavaPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [SULKAVA_CONTACT]
    ...
    const attachments = blockNodes
      ...
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", SULKAVA_LISTING_URL).toString(),
      }))
...
const PUOLANKA_LISTING_URL = "https://puolanka.fi/asuminen-ja-ymparisto/kaavoitus/vireilla-olevat-kaavat/"

const PUOLANKA_CONTACT = {
  name: "Puolangan kunta, kirjaamo",
  title: "Kirjaamo",
  phone: "08 615 5441",
  email: "kunta@puolanka.fi",
}

function puolankaSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function puolankaPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectPuolankaKaavaSource(source: DiscoverySource) {
  const response = await fetch(PUOLANKA_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const headings = $("main, article").first().find("h3").toArray()
  ...
  for (const headingEl of headings) {
    ...
    const phase = puolankaPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [PUOLANKA_CONTACT]
    ...
    const attachments = blockNodes
      ...
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", PUOLANKA_LISTING_URL).toString(),
      }))

    found += 1

    const slug = puolankaSlug(title)
    const documentUrl = `${PUOLANKA_LISTING_URL}#${slug}`
...
const VETELI_LISTING_URL = "https://veteli.fi/asuminen-ja-ymparisto/kaavoitus/"

const VETELI_CONTACT = {
  name: "Vetelin kunta, tekninen toimisto",
  title: "Tekninen toimisto",
  phone: "050 5790 100",
  email: "tekninen@veteli.fi",
}

function veteliSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function veteliPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
async function collectVeteliKaavaSource(source: DiscoverySource) {
  const response = await fetch(VETELI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const headings = $("main, article").first().find("h2").toArray()
  ...
  for (const headingEl of headings) {
    ...
    const phase = veteliPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [VETELI_CONTACT]
    ...
    const attachments = blockNodes
      ...
      .map((a) => ({
        label: $(a).text().replace(/\s+/g, " ").trim(),
        url: new URL($(a).attr("href") ?? "", VETELI_LISTING_URL).toString(),
      }))

    found += 1

    const slug = veteliSlug(title)
    const documentUrl = `${VETELI_LISTING_URL}#${slug}`
...
const VIROLAHTI_LISTING_URL = "https://virolahti.fi/asuminen-ja-ymparisto/kaavoitus/vireilla-olevat-kaavat/"

const VIROLAHTI_CONTACT = {
  name: "Ilkka Jantunen",
  title: "Tekninen johtaja",
  phone: "040 199 1222",
  email: "ilkka.jantunen@virolahti.fi",
}

function virolahtiSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function virolahtiPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
// Virolahti embeds each pending kaava as a WordPress wp:embed block
// linking to the item's own dated blog post; currently only osayleiskaava
// items are listed (filtered out below), but the same embed markup would
// apply to a future asemakaava post.
async function collectVirolahtiKaavaSource(source: DiscoverySource) {
  const response = await fetch(VIROLAHTI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }

  const $ = cheerio.load(await response.text())

  const items = $("blockquote.wp-embedded-content a")
    .toArray()
    .map((el) => ({
      title: $(el).text().replace(/\s+/g, " ").trim(),
      href: $(el).attr("href") ?? "",
  ...
  for (const item of items) {
    const detailUrl = new URL(item.href, VIROLAHTI_LISTING_URL).toString()
    const detailResponse = await fetch(detailUrl, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
    if (!detailResponse.ok) continue
    ...
    const title = item.title
    const description = main.text().replace(/\s+/g, " ").trim()
    const phase = virolahtiPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [VIROLAHTI_CONTACT]
    ...
    const slug = virolahtiSlug(title)
    const rawText = JSON.stringify({ title, phase, description, contacts, attachments })
    const contentHash = hashContent(rawText)
...
const INARI_LISTING_URL = "https://www.inari.fi/fi/palvelut/tekniset-palvelut/kaavoitus/vireilla-olevat-kaavat.html"
...
function inariSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
// Inari lists each item's whole history inline in strict chronological
// order, including occasional reversals ("Valtuusto palautti kaavaehdotuksen
// uudelleen valmisteluun" — sent back to an earlier stage). The standard
// fixed keyword-priority regex would latch onto the highest-priority
// keyword regardless of where it falls in that history; here we instead
// take whichever valid signal appears LAST (rightmost) in the text, since
// that best matches this site's narrative structure.
function inariPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()

  const candidates: { index: number; phase: string }[] = []

  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(normalized)
  const lainvoimaIndex = normalized.lastIndexOf("voimaantulo")
  const lainvoimaIndex2 = normalized.lastIndexOf("lainvoima")
  const voimaantuloIndex = Math.max(lainvoimaIndex, lainvoimaIndex2)
  if (voimaantuloIndex >= 0 && !negatedLainvoima) {
    const isHistoricalYearReference = /\b(19|20)\d{2}\b/.test(
      normalized.slice(Math.max(0, voimaantuloIndex - 40), voimaantuloIndex)
    )
    const isAttachmentReference = /(^|[^a-zäöå])ote\s*$/.test(
      normalized.slice(Math.max(0, voimaantuloIndex - 20), voimaantuloIndex)
    )
    const isBuildingBanReference = /rakennuskiel/.test(
      normalized.slice(Math.max(0, voimaantuloIndex - 25), voimaantuloIndex)
    )
    if (!isHistoricalYearReference && !isAttachmentReference && !isBuildingBanReference) {
      candidates.push({ index: voimaantuloIndex, phase: "Voimaantulo" })
    }
  }
  ...
  if (hyvaksyIndex >= 0) {
    ...
    const isHistoricalBaselineReference = /voimassa oleva|kaavoituskatsau/.test(beforeWindow)
    const isDirectApprovalOfEhdotus = /hyväksy[a-zäöå]*[\s\S]{0,90}?(kaava)?ehdotu(kse|sta)/i.test(window)
    if ((!isForwardLookingOrUnrelated || isDirectApprovalOfEhdotus) && !isHistoricalBaselineReference) {
      candidates.push({ index: hyvaksyIndex, phase: "Hyväksyminen" })
    }
  }

  const ehdotuIndex = normalized.lastIndexOf("ehdotu")
  if (ehdotuIndex >= 0) {
    const ehdotuWindow = normalized.slice(Math.max(0, ehdotuIndex - 60), ehdotuIndex + 200)
    const isForwardLookingEhdotus = /(valmistelussa|valmistelu on (meneillään|käynnissä)|tavoitteena on (asettaa|että))/.test(ehdotuWindow)
    if (!isForwardLookingEhdotus) {
      candidates.push({ index: ehdotuIndex, phase: "Ehdotus" })
    }
  }

  const luonnoIndex = Math.max(normalized.lastIndexOf("luonnos"), normalized.lastIndexOf("luonnok"))
  if (luonnoIndex >= 0) {
    candidates.push({ index: luonnoIndex, phase: "Luonnos" })
  }

  if (candidates.length === 0) return "Vireilletulo"

  candidates.sort((a, b) => b.index - a.index)
  return candidates[0].phase
}

// Inari packs every pending item into one flat page: an <h2> divides
// asemakaava/yleiskaava/ranta-asemakaava sections, and within a section
// each item is a run of <p> content starting at a <strong> title marker
// (multiple items often share one <p>, separated by <br><br>).
async function collectInariKaavaSource(source: DiscoverySource) {
  const response = await fetch(INARI_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }
  ...
  while (node.length && !["h2", "h3"].includes((node.prop("tagName") || "").toLowerCase())) {
  ...
  for (const item of items) {
    const title = item.title
    const description = item.description
    const phase = inariPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [INARI_CONTACT]
    const attachments = item.links
    ...
    const slug = inariSlug(title)
    const documentUrl = `${INARI_LISTING_URL}#${slug}`
    const rawText = JSON.stringify({ title, phase, description, contacts, attachments })
    const contentHash = hashContent(rawText)
...
const MUONIO_LISTING_URL = "https://www.muonio.fi/asuminen-ja-ymparisto/kaavoitus-ja-maankaytto/vireilla-olevat-kaavat.html"

const MUONIO_CONTACT = {
  name: "Aatu Harmaala",
  title: "Maankäyttöinsinööri",
  phone: "040 184 3164",
  email: "aatu.harmaala@muonio.fi",
}

function muonioSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
function muonioPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(
    normalized
  )
...
// Muonio renders each item as an <h2> "card header" bar whose matching
// card body (the actual description + attachment links) is a SIBLING of
// the header's great-grandparent wrapper, not a descendant of the h2 at
// all — confirmed by walking the DOM directly.
async function collectMuonioKaavaSource(source: DiscoverySource) {
  const response = await fetch(MUONIO_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }
  ...
  const items = $("h2")
    ...
    .map((el) => {
      const title = $(el).text().replace(/\s+/g, " ").trim()
      const body = $(el).parent().parent().parent().next()
      return {
        title,
  ...
  for (const item of items) {
    const title = item.title
    const description = item.description
    const phase = muonioPhaseFromText(`${title} ${description}`)
    const completed = phase === "Voimaantulo"
    const contacts = [MUONIO_CONTACT]
    const attachments = item.links
      .filter((l) => l.href.startsWith("http"))
      .map((l) => ({ label: l.label, url: l.href }))
    ...
    const slug = muonioSlug(title)
    const documentUrl = `${MUONIO_LISTING_URL}#${slug}`
    const rawText = JSON.stringify({ title, phase, description, contacts, attachments })
    const contentHash = hashContent(rawText)
...
const HAMEENKYRO_LISTING_URL = "https://hameenkyro.fi/palvelut/asuminen-ja-elinymparisto/kaavoitus-ja-maankaytto/vireilla-ja-nahtavilla-olevat-kaavat/"

const HAMEENKYRO_CONTACT = {
  name: "Anna-Maria Niilo-Rämä",
  title: "Kaavasuunnittelija",
  phone: "044 486 0028",
  email: null as string | null,
}

function hameenkyroSlug(value: string): string {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
...
// Same hardened pattern used across the session, extended with
// "tul(?:lut|leet) voimaan" — Hämeenkyrö's completion wording is "Asemakaava
// on tullut voimaan" (singular) or "Asemakaavan muutokset ovat tulleet
// voimaan" (plural), neither of which contains the bare noun "voimaantulo".
function hameenkyroPhaseFromText(text: string): string {
  const normalized = text.toLowerCase()
  const negatedLainvoima = /(?<![\wäöåÄÖÅ])(ei|eikä)(?![\wäöåÄÖÅ])[^.]{0,40}lainvoima/i.test(normalized)
...
// Each pending plan has its own dedicated sub-page (linked from the index
// page). The sub-page renders its FULL history in reverse-chronological
// order (newest first), with older phases pushed below a
// "Kaavan vanhempia aineistoja" (older materials) <h2> heading — so the
// current phase is isolated by stripping everything from that heading
// onward, rather than relying on keyword-priority scanning of the full text.
async function collectHameenkyroKaavaSource(source: DiscoverySource) {
  const response = await fetch(HAMEENKYRO_LISTING_URL, { cache: "no-store", headers: LOPPI_FETCH_HEADERS })
  if (!response.ok) return { documentsFound: 0, documentsSaved: 0 }
  ...
  for (const item of items) {
    ...
    const wysiwyg = detail$(".wysiwyg").first().clone()
    const olderHeading = wysiwyg
      .find("h2")
      .filter((_: number, el: any) => detail$(el).text().trim() === "Kaavan vanhempia aineistoja")
      .first()
    if (olderHeading.length) {
      olderHeading.nextAll().remove()
      olderHeading.remove()
    }
    const currentText = wysiwyg.text().replace(/\s+/g, " ").trim()

    const phase = hameenkyroPhaseFromText(currentText || title)
    const completed = phase === "Voimaantulo"
    const contacts = [HAMEENKYRO_CONTACT]
    const description = currentText || null
    const attachments = wysiwyg
      .find("a.acf-internal-link")
      .toArray()
      .map((a: any) => ({
    ...
    const slug = hameenkyroSlug(title)
...
export async function collectApiSource(source: DiscoverySource) {
  ...
  if (source.parser === "suomussalmiKaavaParser") {
    return collectSuomussalmiKaavaSource(source)
  }
  ...
  if (source.parser === "enonkoskiKaavaParser") {
    return collectEnonkoskiKaavaSource(source)
  }
