...
export async function fetchSkanskaSource() {
  ...
  for (let page = 0; page < 6; page++) {
    const res = await fetch(
      `https://www.sttinfo.fi/public-website-api/pressroom/69819623/releases/20/${page}`,
      {
        headers: {
          "User-Agent": "Mozilla/5.0",
          "Accept": "application/json, text/plain, */*",
          "Referer": "https://www.sttinfo.fi/embedded/pressroom/69819623/widget/r?language=fi",
        },
      }
    )
